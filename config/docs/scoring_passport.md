...
DecisionMaking._make_decision_for_symbol()
  └─ AuroraScoringKernel.compute()
       ...
       │
       ▼
  └─ EntryPlan Computation
       │
       ▼
  └─ Objective Engine (Score Modulation)
       ├─ [O1] Evaluate Cost (Spread, Fee, Slippage penalties)
       ├─ [O2] Evaluate Risk (Inventory, Volatility penalties)
       ├─ [O3] Evaluate Edge & Execution
       ├─ [O4] Map to Multiplier via Sigmoid Function
       └─ [O5] final_score = kernel_score × multiplier (gate if below min_objective_score)
```
...
### 3.8 Objective Engine Modulation (Оцінка якості)

Після базового скорингу (AuroraScoringKernel) та перевірки EntryPlan застосовується `Objective Engine`, який модулює впевненість сигналу (`signal_score`) без зміни його напрямку (buy/sell).

**Математика Objective Engine:**
```
total_penalty = w_cost × Cost(fees, slippage, spread) 
              + w_risk × Risk(volatility, inventory_utilization)
              + w_edge × Edge(signal_score)
              + w_execution × Execution(liquidity)
              + w_info × Information(staleness)

z_value = total_penalty × lambda_scale
normalized_penalty = sigmoid(z_value, center=-5.0, scale=2.5)
...
**Особливості:**
- Якщо конфіг гейта (`enforcement_mode="GATE"`) активний, і `final_score < min_objective_score`, сигнал отримує **повне блокування** (перетворюється на neutral).
- Ваги штрафів та винагород (`w_cost`, `w_risk` тощо) є **залежними від режиму** (напр. у `HIGH_VOLATILITY` ризик має вищу вагу).
