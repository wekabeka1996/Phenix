...
async def validate_testnet():
    ...
    with patch.object(adapter, 'session') as mock_session, \
        ...
        mock_response.json.return_value = {"orderId": 123, "clientOrderId": "test123"}
    ...
    if live_exchange_info != exchange_info_real:
        differences.append("Фільтри відрізняються: TESTNET tickSize=0.1, LIVE tickSize=0.01 (симульовано)")
