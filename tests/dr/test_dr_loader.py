...
class TestDRLoader:
    ...
    def test_replay_wal_with_events(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            before_ts = int((snapshot_ts - timedelta(minutes=5)).timestamp() * 1_000_000)
            event_before = {
                "op": "EVT",
                "verb": "TRADE_EXECUTED",
                "pld": {
            ...
            account_ts = int((snapshot_ts + timedelta(minutes=10)).timestamp() * 1_000_000)
            event_account = {
                "op": "EVT",
            ...
            # Assertions
            assert count == 2, "Should replay 2 events (1 trade + 1 account update after snapshot)"
            assert mock_fsm.on_trade_executed.call_count == 1, "Should call on_trade_executed once"
            assert mock_fsm.on_account_update.call_count == 1, "Should call on_account_update once"
