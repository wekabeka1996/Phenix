...
class TestCausalGraph:
    ...
    def test_add_transition(self, graph):
        ...
        z2 = np.random.randn(8).astype(np.float32) + \
            2  # Far enough for separate node
...
class TestDreamer:
    ...
    def graph_and_dreamer(self, tmp_path):
        ...
        def encoder(x): return x[:8] if len(
            x) >= 8 else np.pad(x, (0, 8-len(x)))
    ...
    def test_missing_encoder_does_not_create_raw_feature_latent(self, tmp_path):
        """A Dreamer without an encoder must not treat raw features as latent state."""
        db_path = tmp_path / "test_dreamer_no_encoder.db"
        graph = CausalGraph(db_path=db_path, latent_dim=2)
        ...
        result = dreamer.consolidate([
            Episode(features={"f1": 1.0, "f2": 2.0},
                    side="LONG", reward=1.0, timestamp=1.0)
        ])

        assert result["episodes_processed"] == 0
        assert result["transitions_added"] == 0
        graph.close()

    def test_missing_reward_does_not_create_zero_reward_transition(self, graph_and_dreamer):
        """Missing reward must not be converted to a synthetic zero reward."""
        graph, dreamer = graph_and_dreamer
        ...
        result = dreamer.consolidate([
            Episode(features={"f1": 1.0, "f2": 2.0},
                    side="LONG", reward=None, timestamp=1.0)
        ])
