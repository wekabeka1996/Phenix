...
def test_buffer_memory_stability(stress_config):
    ...
    assert min(
        timestamps) == 500.0, f"Oldest should be 500, got {min(timestamps)}"
    assert max(
        timestamps) == 999.0, f"Newest should be 999, got {max(timestamps)}"
...
def test_simulation_1000_ticks(stress_config):
    ...
    mock_bridge.encode_async = AsyncMock(
        return_value=np.random.randn(4).astype(np.float32))
...
def test_no_broken_pool_errors(stress_config):
    ...
    mock_bridge.train_async = AsyncMock(
        return_value={"vae_loss": 0.1, "wm_loss": 0.1})
