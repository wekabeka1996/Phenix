...
class SyntheticDataGenerator:
    ...
    def generate_bar_features(self, symbol: str, step: int) -> Dict[str, Any]:
        ...
        mean_revert = -0.001 * (prev_price / self.price_state[symbol] - 1)
...
class TestSimulationBarV2:
    ...
    def test_full_pipeline_with_learning(self, sim_config, temp_workspace):
        ...
        async def run_simulation():
            ...
            for step in range(60, 60 + num_steps):  # Continue from warmup
                ...
                for symbol in ["BTCUSDT", "ETHUSDT"]:
                    ...
                    log_path = temp_workspace / "logs" / \
                        "features" / f"{symbol}.log"
                ...
                if step % 40 == 0:
                    ...
                    write_core_log(
                        core_path, generator.generate_position_closed(symbol, step, pnl))
                    write_core_log(
                        core_path, generator.generate_equity_update(step, equity))
                    positions_closed += 1
        ...
        shadow_events_legacy = [
            e for e in emitted_events if e[0] == "EVT:NEOCORTEX_SHADOW_INTENT"]
        shadow_events_canonical = [
            e for e in emitted_events if e[0] == "EVT:NEOCORTEX_SHADOW_INTENT_PROPOSED"]
        ...
        if checkpoints:
            ...
            print(
                f"Training steps in checkpoint: {cp_data.get('train_steps', 'N/A')}")
