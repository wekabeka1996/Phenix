    ...
    def emit(self, event_name: str, payload: dict, why: str) -> None:
        if event_name in self.listeners:
            for cb in self.listeners[event_name]:
                cb(Message(op="EVT", verb=event_name.split(":")[1], src="test", dst="any", pld=payload, why=why))
...
class TestMarketDataConnectorIsolation:
    ...
    def test_init_without_unicorn_dependency(self, monkeypatch, mock_config):
        ...
        with mock.patch('apps.reference.domains.market_data.market_data_connector.BinanceAdapter') as mock_adapter:
    ...
    def test_sequence_control_depth_update(self, mock_config):
        ...
        with mock.patch('apps.reference.domains.market_data.market_data_connector.BinanceAdapter') as mock_adapter_class:
            ...
            custom_config['system'] = {'trading': {'symbols_to_track': ['BTCUSDT', 'ETHUSDT']}}
