...
async def test_three_domain_chain_integration(mock_fsm, mock_config):
    ...
    with patch(
        ...
        mock_adapter.get_book_ticker.return_value = {
            "bidPrice": "50000",
            "bidQty": "10",
            "askPrice": "50002",
            "askQty": "8",
        ...
        feature_engineering = FeatureEngineering(
            fsm=mock_fsm, config=mock_config.get("trading", {})
        )
        risk_management = RiskManagement(
            fsm=mock_fsm, config=mock_config.get("system", {})
        )

        try:
            # Now patch the adapter instance method get_klines to return a valid recent kline
            with patch.object(
                market_data.adapter, "get_klines", new_callable=AsyncMock
            ) as mock_get_klines:
                mock_kline = [
