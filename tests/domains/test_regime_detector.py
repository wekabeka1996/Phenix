...
regime_detector_path = project_root / "apps" / "reference" / "domains" / "regime_detector" / "regime_detector.py"

spec = importlib.util.spec_from_file_location("regime_detector_module", regime_detector_path)
...
def test_detects_trend_up_regime_on_clear_signal(mock_config, mock_fsm_core):
    ...
    assert Decimal(emitted_payload["confidence"]) > Decimal("0.7"), "Confidence should be high for clear signal"
...
def test_detects_trend_down_regime_on_clear_signal(mock_config, mock_fsm_core):
    ...
    assert emitted_event_name == "EVT:REGIME_DETECTED"
    assert emitted_payload["symbol"] == "ETHUSDT"
    assert emitted_payload["regime"] == "TREND_DOWN", "Expected TREND_DOWN for bearish signal"
    assert Decimal(emitted_payload["confidence"]) > Decimal("0.7"), "Confidence should be high for clear downtrend"
    assert emitted_payload["source_model"] == "sma_trend_v1"
...
def test_detects_mean_reversion_regime_when_price_is_close_to_smas(mock_config, mock_fsm_core):
    ...
    assert emitted_event_name == "EVT:REGIME_DETECTED"
    assert emitted_payload["symbol"] == "ETHUSDT"
    assert emitted_payload["regime"] == "MEAN_REVERSION", "Expected MEAN_REVERSION for ranging market"
    assert Decimal(emitted_payload["confidence"]) > Decimal("0.8"), "Confidence should be very high for tight convergence"
    assert emitted_payload["source_model"] == "sma_trend_v1"
