...
def test_simulated_place_and_cancel_and_status():
    ...
    dec = Message(op="DEC", verb="OPEN", src="t", dst="sim", rid="r1", pld={"symbol": "ETHUSDT", "qty": "0.5", "side": "BUY", "price": "100"})
    res = adapter.place_order(dec)
    assert res["status"] == "ACCEPTED"
    assert res["exchange_order_id"].startswith("sim_")

    cancel_msg = Message(op="DEC", verb="CANCEL", src="t", dst="sim", rid="r2", pld={"exchange_order_id": res["exchange_order_id"]})
...
def test_simulated_adapter_cancel_order(adapter):
    ...
    dec_msg = Message(
        ...
        pld={
            "exchange_order_id": "sim_1234567890",
            "symbol": "BTCUSDT"
        }
    )
