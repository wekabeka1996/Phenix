...
class TestCollectLeverageConfigsFromInstruments:
    ...
    def test_returns_leverage_config_objects(self, fsm_instance):
        ...
        for symbol, cfg in result.items():
            assert isinstance(
                cfg, LeverageConfig), f"{symbol} should be LeverageConfig"
            assert cfg.mode in (
                "ISOLATED", "CROSSED"), f"{symbol} mode should be valid"
...
class TestLeverageSSOTConsistencyValidation:
    ...
    def test_no_warnings_when_consistent(self, fsm_instance, mock_strategy_leverage):
        ...
        mock_strategy_leverage["BTCUSDT"].leverage = LeverageConfig(
            target=50,
            mode="ISOLATED",
            max_notional_value=None,
        )
        mock_strategy_leverage["ETHUSDT"].leverage = LeverageConfig(
            target=41,
            mode="ISOLATED",
            max_notional_value=None,
        )
...
class TestLeverageBootstrapIntegration:
    ...
    def test_bootstrap_collects_instruments_leverage(self, fsm_instance):
        ...
        assert leverage_configs[
            "BTCUSDT"].target == 50, "Bootstrap should use instruments.yaml value (50), not strategy (20)"
        assert leverage_configs[
            "ETHUSDT"].target == 41, "Bootstrap should use instruments.yaml value (41), not strategy (20)"
...
class TestLeverageEdgeCases:
    ...
    def test_symbol_in_assignments_but_not_instruments(self, fsm_instance):
        ...
        fsm_instance.config.strategies_registry.assignments["NEWUSDT"] = [
            "aurora"]
    ...
    def test_multiple_strategies_same_symbol(self, fsm_instance):
        ...
        fsm_instance.config.strategies_registry.assignments["BTCUSDT"] = [
            "aurora", "mean_reversion"]
