...
class TestNoLegacyRuntime:
    ...
    def test_legacy_manage_flow_marked_deprecated(self):
        """Ensure legacy ManageFlowFSM is flagged as deprecated (not prod path)."""
        import apps.reference.domains.execution_position.legacy.fsm_manage as fm
        assert getattr(fm, "LEGACY_OCO_DEPRECATED", False) is True
