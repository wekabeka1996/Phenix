...
def fsm_config():
    ...
    cfg.domains.execution_position.guardian = guardian
...
async def test_fsm_fill_before_ack_out_of_order(exec_pos_fsm):
    ...
    fill_msg = Message(
        ...
        pld={"orderId": "ord_1", "symbol": "BTCUSDT",
             "quantity": "0.1", "rid": "r1"}
    )
    ack_msg = Message(
...
def test_fsm_deduper_eviction_under_cap(exec_pos_fsm):
    ...
    exec_pos_fsm._processed_events = BoundedEventDeduper(
        max_size=2, ttl_ms=100000)
...
def test_fsm_on_portfolio_state_malformed(exec_pos_fsm):
    ...
    msg = Message(op="EVT", verb="PORTFOLIO_STATE_UPDATED",
                  src="src", dst="dst", pld={})
...
async def test_fsm_idempotent_cancel_no_retry_storm(exec_pos_fsm):
    ...
    exec_pos_fsm.adapter.cancel_order = AsyncMock(
...
def test_fsm_ack_missing_fields_fail_closed(exec_pos_fsm):
    ...
    ack_msg = Message(op="EVT", verb="ORDER_ACK", src="src",
                      dst="dst", pld={"symbol": "BTCUSDT"})
    exec_pos_fsm._on_order_ack(ack_msg)
    exec_pos_fsm.watchdog.on_order_ack.assert_not_called()
...
def test_fsm_fill_missing_fields_fail_closed(exec_pos_fsm):
    ...
    fill_msg = Message(op="EVT", verb="ORDER_FILL", src="src", dst="dst", pld={
                       "orderId": "f1", "symbol": "BTCUSDT"})
    exec_pos_fsm._on_order_fill(fill_msg)
...
def test_trade_executed_marks_order_index_terminal(exec_pos_fsm):
    ...
    exec_pos_fsm._get_or_create_flows = MagicMock(
        return_value=(open_flow, manage_flow, close_flow))
...
def test_fsm_is_cancel_success_all_cases(exec_pos_fsm):
    ...
    assert isc(IdempotentCancelResult(success=False,
               reason="fail", is_idempotent_success=True)) is False
...
def test_fsm_resolve_guardian_config_branches(exec_pos_fsm):
    ...
    assert res["emit_tidy_event"] is True
    assert res["emit_tidy_monitoring_event"] is True
...
def test_fsm_cleanup_enabled_missing_explicit_config_raises(exec_pos_fsm):
    ...
    with pytest.raises(ValueError, match="fsm_periodic_cleanup_enabled"):
        exec_pos_fsm._resolve_fsm_periodic_cleanup_enabled()
...
def test_fsm_on_order_fill_intent_injection(exec_pos_fsm):
    ...
    rid = "r_intent"
    exec_pos_fsm._pending_intent_data[rid] = {
        "stop_price": 40000.0, "target_price": 60000.0}
    ...
    exec_pos_fsm._on_order_fill(fill_msg)
    mock_flow.set_intent_prices.assert_called_with(
        sl_price=40000.0, tp_price=60000.0)
...
async def test_fsm_handle_timeout_cancel_rejected(exec_pos_fsm):
    ...
    exec_pos_fsm._cancel_order = AsyncMock(
        return_value={"status": "REJECTED", "code": -123})
...
async def test_fsm_execute_decision_safety_guardrail(exec_pos_fsm):
    ...
    msg = Message(op="DEC", verb="CANCEL_ORDER", src="s", dst="d",
                  pld={"orderId": "1", "symbol": "BTCUSDT"})
...
async def test_fsm_execute_decision_close_full_flow(exec_pos_fsm):
    ...
    symbol = "BTCUSDT"
    msg = Message(op="DEC", verb="CLOSE", src="s", dst="d",
                  pld={"symbol": symbol}, rid="close_123")

    # 1. Brackets cancellation setup
    exec_pos_fsm._symbol_brackets[symbol] = {
    ...
    exec_pos_fsm.adapter.get_open_positions = AsyncMock(
    ...
    # sl1, tp1 and potentially leftovers
    assert exec_pos_fsm._cancel_order.call_count >= 2
...
async def test_fsm_execute_decision_cancel_error(exec_pos_fsm):
    ...
    msg = Message(op="DEC", verb="CANCEL_ORDER", src="s", dst="d",
                  pld={"orderId": "bad_id", "symbol": "ETHUSDT"})
...
async def test_fsm_execute_decision_cancel_routing(exec_pos_fsm):
    ...
    msg = Message(op="DEC", verb="CANCEL_ORDER", src="s", dst="d",
                  pld={"orderId": "ord_cancel", "symbol": "BTCUSDT"})
...
def test_fsm_metrics_expose_resolved_guardian_tidy_monitoring_flag(exec_pos_fsm):
    exec_pos_fsm._guardian_emit_tidy_event = False
    exec_pos_fsm._guardian_emit_tidy_monitoring_event = False

    metrics = exec_pos_fsm.get_metrics()

    assert metrics["guardian_emit_tidy_event"] is False
    assert metrics["guardian_emit_tidy_monitoring_event"] is False
...
def test_fsm_entry_tidy_gate_uses_symbol_tidy_when_monitoring_event_disabled(exec_pos_fsm):
    exec_pos_fsm.config = MagicMock()
    exec_pos_fsm.config.execution.allow_trade_with_guardian_tidy_only = True
    exec_pos_fsm._guardian_emit_tidy_monitoring_event = False
    ...
    assert symbol in exec_pos_fsm._symbol_last_tidy_ts
    assert exec_pos_fsm._entry_tidy_gate_allow(symbol) is True
...
async def test_fsm_execute_decision_open_with_backoff(exec_pos_fsm):
    ...
    exec_pos_fsm.adapter.place_market_entry = AsyncMock(
        return_value=entry_resp)
    exec_pos_fsm.adapter.place_stop_market_close_position = AsyncMock(return_value={
                                                                      "orderId": 67890})

    # Mock TP to fail with -2021 once, then succeed
    from apps.reference.adapters.binance_adapter import BinanceAPIError
    err_2021 = BinanceAPIError(
        code=-2021, msg="Order would immediately trigger")
    # Ensure TP response uses numeric ID
    exec_pos_fsm.adapter.place_take_profit_market_close_position = AsyncMock(
        side_effect=[err_2021, {"orderId": 11111}])

    exec_pos_fsm._bracket_mgr.preflight_position_check = AsyncMock(
        return_value=True)
    exec_pos_fsm.order_guardian.should_place_brackets = AsyncMock(
        return_value=True)
...
async def test_fsm_execute_decision_place_order(exec_pos_fsm):
    ...
    exec_pos_fsm.adapter.place_limit_reduce_only = AsyncMock(
        return_value={"orderId": 55555})
