...
def mock_config():
    ...
    config.binance_api = MagicMock()
    config.strategies_registry = MagicMock()
    ...
    config.trading.execution.fsm_periodic_cleanup_enabled = False
    config.trading.execution.watchdog.ack_ttl_ms = 5000
    config.trading.execution.watchdog.fill_ttl_ms = 30000
    config.trading.execution.watchdog.check_interval_ms = 1000
    config.trading.execution.watchdog.rps_limit = 10
    config.trading.execution.manage.orphan_monitor = {}
    config.trading.execution.order_guardian = {"unified": False}
    config.trading.mode = "testnet"

    guardian = config.domains.execution_position.guardian
...
def test_collect_leverage_configs_returns_empty_without_registry(mock_config, mock_fsm):
...
async def test_run_leverage_bootstrap_calls_bootstrapper(mock_config, mock_fsm):
    ...
    mock_results.add_success("BTCUSDT", BootstrapResult(
        symbol="BTCUSDT", success=True))
    mock_results.add_success("ETHUSDT", BootstrapResult(
        symbol="ETHUSDT", success=True))
    mock_results.add_success("DOGEUSDT", BootstrapResult(
        symbol="DOGEUSDT", success=True))
