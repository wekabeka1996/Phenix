...
class MockFSM:
    def __init__(self, shadow_mode=False, loop=None):
        ...
        self.fsm = MagicMock()
        self.fsm._emit_compat_mode = "message"
        self.fsm.emit = MagicMock()
        self.fsm.emit._emit_compat_mode = "message"
    ...
    def dequeue_supersede(self, symbol: str):
...
async def test_cancel_pending_entries_async_success(entry_manager, fsm):
    tasks = []
    fsm._submit_async = lambda coro, loop: tasks.append(
        asyncio.create_task(coro))

    fsm.watchdog.pending_orders = {"o1": MockDeadline("BTCUSDT", "o1")}
    fsm._pending_entry_meta = {"o1": "meta"}

    entry_manager.cancel_pending_entries_for_symbol(
        "BTCUSDT", "ASYNC_CANCEL", filter_order_ids={"o1"})
