...
def fsm_config():
    ...
    cfg.trading.exposure.leverage_defaults = {"__default__": "20", "BTCUSDT": "20"}
...
def fsm_harness(fsm_config):
    ...
    with patch("apps.reference.domains.execution_position.fsm.OrderGuardian") as MockGuardian:
        ...
        fsm = ExecPosFSM(config=fsm_config, fsm=mock_core_fsm, shadow_mode=True)
...
def test_diag_reject_below_min_notional(fsm_config):
    ...
    assert "notional" in str(result.why).lower() or "notional" in str(result.pld.get("reason")).lower()
...
def test_diag_partial_fill_accounting():
    ...
    msg = Message(
        ...
        pld={"symbol": "BTCUSDT", "qty": "0.5"} # Assume target was 1.0 (context implied)
    )
...
def test_diag_reduce_only_close_safety():
    ...
    msg = Message(op="CMD", verb="CLOSE", src="dec", dst="exec", pld={"symbol": "BTCUSDT"})
    ...
    assert res.pld.get("reduce_only") is True, "DEC:CLOSE must initiate with reduce_only=True"
