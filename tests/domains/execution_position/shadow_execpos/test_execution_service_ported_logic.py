...
async def test_place_order_success(service, mock_adapter):
    ...
    assert isinstance(params, ExchangeOrderParams)
...
async def test_place_order_would_trigger_error(service, mock_adapter):
    ...
    exc = Exception(f"APIError: code={ERROR_WOULD_TRIGGER}, msg=stopPrice would trigger immediately")
    mock_adapter.create_order.side_effect = exc
...
async def test_place_order_duplicate_id_error(service, mock_adapter):
    ...
    exc = Exception(f"APIError: code={ERROR_DUPLICATE_ID}, msg=Duplicate clientOrderId")
    mock_adapter.create_order.side_effect = exc
...
async def test_close_position_success(service, mock_adapter):
    ...
    # Verify reduce_only MARKET order was placed via create_order
    assert mock_adapter.create_order.called
    call_args = mock_adapter.create_order.call_args
    params = call_args.kwargs.get('params')
    assert isinstance(params, ExchangeOrderParams)
...
async def test_call_adapter_nonexistent_method(service, mock_adapter):
    ...
    real_adapter_mock = MagicMock(spec=["create_order", "cancel_order"])
