...
# Phase 11: runtime uses compute_bracket_plan_from_views loop instead of bracket_service.evaluate_all_for_recovery
pytestmark = pytest.mark.xfail(
    reason="Phase 11: Legacy bracket_service mock - runtime uses compute_bracket_plan_from_views")
...
async def test_recovery_orphan_cleanup_calls_apply_once():
    runtime = ExecPosRuntimeV2(
        config={}, adapter=None, price_service=None, guardian=None)
    runtime.bracket_service = MagicMock()
    plan = make_plan("BTCUSDT", "LONG", [BracketAction(
        action="CANCEL", order_ref="orph")], severity="WARN")
    ...
    runtime._open_orders_by_symbol = {"BTCUSDT": [
        {"symbol": "BTCUSDT", "orderId": "orph", "type": "STOP_MARKET", "side": "SELL", "reduceOnly": True}]}

    await runtime._run_bracket_recovery_pass()

    runtime.bracket_service.evaluate_all_for_recovery.assert_called_once()
    runtime._apply_bracket_plan.assert_awaited_once_with("BTCUSDT", PositionState(
        symbol="BTCUSDT"), plan, reason="brackets_recovery_orphan_cleanup")
...
async def test_recovery_seed_protection_places_missing_sl():
    runtime = ExecPosRuntimeV2(
        config={}, adapter=None, price_service=None, guardian=None)
    runtime.bracket_service = MagicMock()
    actions = [BracketAction(action="PLACE_SL", leg_type="SL", target_price=Decimal(
        "95"), qty=Decimal("100"), reason_code="MISSING_SL")]
    plan = make_plan("ETHUSDT", "LONG", actions, severity="ALERT")
    runtime.bracket_service.evaluate_all_for_recovery = MagicMock(return_value=[
                                                                  plan])
    runtime._apply_bracket_plan = AsyncMock()
    runtime._positions_by_symbol = {"ETHUSDT": PositionState(
        symbol="ETHUSDT", qty=100.0, avg_entry_price=100.0)}
    runtime._open_orders_by_symbol = {}

    await runtime._run_bracket_recovery_pass()

    runtime._apply_bracket_plan.assert_awaited_once()
    assert runtime._apply_bracket_plan.await_args.kwargs[
...
async def test_recovery_adjust_mismatch():
    runtime = ExecPosRuntimeV2(
        config={}, adapter=None, price_service=None, guardian=None)
    runtime.bracket_service = MagicMock()
    actions = [BracketAction(action="ADJUST", order_ref="old_tp", target_price=Decimal(
        "120"), qty=Decimal("50"), reason_code="STALE_LEVELS")]
    plan = make_plan("SOLUSDT", "LONG", actions, severity="WARN")
    runtime.bracket_service.evaluate_all_for_recovery = MagicMock(return_value=[
                                                                  plan])
    runtime._apply_bracket_plan = AsyncMock()
    runtime._positions_by_symbol = {"SOLUSDT": PositionState(
        symbol="SOLUSDT", qty=50.0, avg_entry_price=100.0)}
    runtime._open_orders_by_symbol = {"SOLUSDT": [
        {"symbol": "SOLUSDT", "orderId": "old_tp", "type": "TAKE_PROFIT_MARKET", "side": "SELL", "reduceOnly": True}]}
