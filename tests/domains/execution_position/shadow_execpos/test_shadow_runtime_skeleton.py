...
async def test_runtime_instantiation(runtime):
    ...
    assert runtime.price_enricher is not None
...
@pytest.mark.asyncio
async def test_async_manager_stub():
    """Verify AsyncManager stub methods."""
    ...
    # Submit stub
    task = manager.submit(asyncio.sleep(0))
    # Note: Stub currently returns None, but should eventually return a Task
    
    # Shutdown stub
    manager.shutdown_background_tasks()
