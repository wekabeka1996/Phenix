...
async def test_brackets_recovery_blocks_then_recovers_on_first_snapshot():
    ...
    position = PositionState(symbol=symbol, qty=1.0, avg_entry_price=2000.0, cycle_id=1)
    ...
    recovery_plan = make_plan(symbol, "LONG", [place_sl_action, place_tp_action], severity="ALERT", why="recovery_missing_brackets")

    runtime.bracket_service = MagicMock()
    runtime.bracket_service.evaluate_all_for_recovery = MagicMock(return_value=[recovery_plan])
    runtime._apply_bracket_plan = AsyncMock()

    # Act 2: Simulate ORDERS_SNAPSHOT arrival and recovery pass
    runtime.order_index.reconcile_snapshot(symbol, [])  # Empty (no existing brackets)
...
async def test_duplicate_sl_with_unknown_snapshot_not_cleaned_until_trade():
    ...
    position = PositionState(symbol=symbol, qty=0.5, avg_entry_price=50000.0, cycle_id=1)
    runtime._positions_by_symbol[symbol] = position
    runtime._orders_snapshot_state[symbol] = "UNKNOWN"

    # Two reduce_only STOP orders (duplicate SL scenario)
    sl_order_1 = make_stop_order(symbol, "sl-1", "SELL", 0.5, 49000.0, client_order_id="AUR-BTCUSDT-LONG-PLACE_SL-C1-a")
    sl_order_2 = make_stop_order(symbol, "sl-2", "SELL", 0.5, 49000.0, client_order_id="AUR-BTCUSDT-LONG-PLACE_SL-C1-b")
    ...
    # Arrange for trade: Mock bracket_service.evaluate to return CANCEL for extra SL
    cancel_action = BracketAction(
        action_type="CANCEL",
        order_id="sl-2",
        reason_code="TOO_MANY_SL",
        why="duplicate_sl_cleanup",
    )
    cleanup_plan = make_plan(symbol, "LONG", [cancel_action], severity="WARN", why="too_many_sl")
    ...
    runtime.bracket_service.build_state = MagicMock(return_value={
        (symbol, "LONG"): BracketState(
            symbol=symbol,
            side="LONG",
            position_view=PositionView(symbol=symbol, side="LONG", qty=Decimal("0.5"), avg_entry_price=Decimal("50000")),
            bracket_set=None,
            guardian_meta=None,
        )
    })
    runtime.bracket_service.evaluate = MagicMock(return_value=cleanup_plan)
...
async def test_partial_close_while_awaiting_snapshot_resizes_after_snapshot():
    ...
    position = PositionState(symbol=symbol, qty=2.0, avg_entry_price=100.0, cycle_id=1)
    runtime._positions_by_symbol[symbol] = position
    runtime._orders_snapshot_state[symbol] = "STALE"

    sl_order = make_stop_order(symbol, "sl-orig", "SELL", 2.0, 95.0, client_order_id="AUR-SOLUSDT-LONG-PLACE_SL-C1")
    ...
    runtime._bracket_status[symbol] = BracketStatus(awaiting_snapshot=True, last_reason="partial_close")

    # Act 1: Simulate two partial closes (qty goes 2.0 → 1.5 → 1.0)
    position = PositionState(symbol=symbol, qty=1.0, avg_entry_price=100.0, cycle_id=1)
    runtime._positions_by_symbol[symbol] = position
    ...
    # Arrange for snapshot: Mock bracket_service to return ADJUST action
    adjust_action = BracketAction(
        action_type="CANCEL",
        order_id="sl-orig",
        reason_code="SIZE_MISMATCH",
        why="sl_qty_mismatch",
    )
    place_sl_resized = BracketAction(
        action_type="PLACE_SL",
        price=Decimal("95"),
        qty=Decimal("1.0"),
        reason_code="RESIZE_SL",
    )
    resize_plan = make_plan(symbol, "LONG", [adjust_action, place_sl_resized], severity="WARN", why="resize_brackets")
    ...
    runtime.bracket_service.build_state = MagicMock(return_value={
        (symbol, "LONG"): BracketState(
            symbol=symbol,
            side="LONG",
            position_view=PositionView(symbol=symbol, side="LONG", qty=Decimal("1.0"), avg_entry_price=Decimal("100")),
            bracket_set=None,
            guardian_meta=None,
        )
    })
    runtime.bracket_service.evaluate = MagicMock(return_value=resize_plan)

    # Act 2: Fresh ORDERS_SNAPSHOT clears awaiting_snapshot
    runtime._bracket_status[symbol].awaiting_snapshot = False
    runtime._orders_snapshot_state[symbol] = "FRESH"
    runtime._last_orders_snapshot_ts[symbol] = time.time()
    runtime.order_index.reconcile_snapshot(symbol, [sl_order])  # Still has old 2.0 qty SL

    await runtime._evaluate_brackets(symbol, position, reason="trade_executed")

    # Assert 2: SL was cancelled and new one placed
    runtime.execution_service.cancel_order.assert_awaited()
    runtime.execution_service.place_order.assert_awaited()

    # Verify place_order was called with qty=1.0
    place_call = runtime.execution_service.place_order.call_args
    assert float(place_call.kwargs.get("quantity")) == 1.0
...
async def test_allow_unprotected_position_honors_missing_sl():
    ...
    position = PositionState(symbol=symbol, qty=100.0, avg_entry_price=0.5, cycle_id=1)
    runtime._positions_by_symbol[symbol] = position
    runtime._orders_snapshot_state[symbol] = "FRESH"
    runtime._last_orders_snapshot_ts[symbol] = time.time()
    runtime.order_index.reconcile_snapshot(symbol, [])  # No SL/TP
    ...
    info_plan = make_plan(symbol, "LONG", [], severity="INFO", why="unprotected_position_allowed")
    ...
    runtime.bracket_service.build_state = MagicMock(return_value={
        (symbol, "LONG"): BracketState(
            symbol=symbol,
            side="LONG",
            position_view=PositionView(symbol=symbol, side="LONG", qty=Decimal("100"), avg_entry_price=Decimal("0.5")),
            bracket_set=None,
            guardian_meta=None,
        )
    })
...
async def test_guardian_v2_compat_mode_blocks_autoheal():
    ...
    position = PositionState(symbol=symbol, qty=5.0, avg_entry_price=30.0, cycle_id=1)
    runtime._positions_by_symbol[symbol] = position
    runtime._orders_snapshot_state[symbol] = "FRESH"
    runtime._last_orders_snapshot_ts[symbol] = time.time()
    ...
    runtime.bracket_service.build_state = MagicMock(return_value={
        (symbol, "LONG"): BracketState(
            symbol=symbol,
            side="LONG",
            position_view=PositionView(symbol=symbol, side="LONG", qty=Decimal("5"), avg_entry_price=Decimal("30")),
            bracket_set=None,
            guardian_meta=None,
        )
    })
...
async def test_guard_loop_respects_snapshot_freshness_ttl():
    ...
    position = PositionState(symbol=symbol, qty=10.0, avg_entry_price=15.0, cycle_id=1)
    runtime._positions_by_symbol[symbol] = position

    # Set snapshot as FRESH but with old timestamp (simulating staleness)
    runtime._orders_snapshot_state[symbol] = "FRESH"
    runtime._last_orders_snapshot_ts[symbol] = time.time() - 120  # 2 minutes ago (> 60s TTL)
