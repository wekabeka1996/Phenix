...
LOCATION: This is a TEST UTILITY, not business logic.
...
class AdapterCall:
    """
    Record of an adapter call for comparison.

    Used to define expected behavior and capture actual calls.
    All fields except verb/symbol are optional to allow partial matching.
    """
...
def diff_adapter_calls(
    ...
    for i in range(min_len):
        ...
        # FIX: Also compare client_order_id and order_id when specified
        if exp.client_order_id is not None and exp.client_order_id != act.client_order_id:
            differences.append(
                f"client_order_id: expected={exp.client_order_id}, actual={act.client_order_id}"
            )

        if exp.order_id is not None and exp.order_id != act.order_id:
            differences.append(
                f"order_id: expected={exp.order_id}, actual={act.order_id}"
            )
...
class ExecPosReplay:
    ...
    async def run(
        ...
        # Extract actual adapter calls from FakeRecordingAdapter
