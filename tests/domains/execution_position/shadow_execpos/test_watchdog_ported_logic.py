...
def test_analyze_no_violations(watchdog):
    """Test valid state - position with one SL -> INFO severity, no recs."""
    positions = [{"symbol": "BTCUSDT", "positionAmt": "0.1", "side": "BUY", "entryPrice": "100"}]
    orders = [
        {
            "symbol": "BTCUSDT",
            "orderId": "sl_123",
...
def test_no_sl_for_open_position(watchdog):
    """Test position without SL protection produces ALERT severity."""
    positions = [{"symbol": "ETHUSDT", "positionAmt": "1.5", "entryPrice": "1800"}]
    orders = []  # No SL order

    recommendations = watchdog.analyze(orders, positions)

    assert recommendations
    assert any(rec.symbol == "ETHUSDT" and rec.kind == "ALERT" for rec in recommendations)
    assert all(rec.action == WatchdogAction.ALERT for rec in recommendations)
...
def test_too_many_sl_orders(watchdog):
    ...
    positions = [{"symbol": "SOLUSDT", "positionAmt": "10.0", "entryPrice": "50"}]
    orders = [
        {"symbol": "SOLUSDT", "orderId": "sl_1", "type": "STOP_MARKET", "side": "SELL", "reduceOnly": True, "stopPrice": "45", "quantity": "10"},
        {"symbol": "SOLUSDT", "orderId": "sl_2", "type": "STOP_MARKET", "side": "SELL", "reduceOnly": True, "stopPrice": "44", "quantity": "10"},
        {"symbol": "SOLUSDT", "orderId": "sl_3", "type": "STOP_MARKET", "side": "SELL", "reduceOnly": True, "stopPrice": "43", "quantity": "10"},
    ]
    ...
    assert recommendations
    assert any(rec.kind == "WARN" for rec in recommendations)
    assert any("sl_2" in rec.orders_to_cancel or "sl_3" in rec.orders_to_cancel for rec in recommendations)
...
def test_multiple_symbols(watchdog):
    ...
    positions = [
        {"symbol": "BTCUSDT", "positionAmt": "0.1", "entryPrice": "100"},  # Has SL - OK
        {"symbol": "ETHUSDT", "positionAmt": "1.0", "entryPrice": "1800"},   # No SL - violation
    ]
    orders = [
        {"symbol": "BTCUSDT", "orderId": "sl_btc", "type": "STOP_MARKET", "side": "SELL", "reduceOnly": True, "stopPrice": "98", "quantity": "0.1"}
        # No ETH SL
    ]
...
def test_position_normalization_various_formats(watchdog):
    """Test position normalization handles various formats and surfaces missing SL."""
    positions = [
    ...
    assert all(rec.kind == "ALERT" for rec in recommendations)
...
def test_order_normalization_stop_detection(watchdog):
    ...
    orders = [
        {"symbol": "BTCUSDT", "orderId": "sl_1", "type": "STOP_MARKET", "side": "SELL", "reduceOnly": True, "stopPrice": "95", "quantity": "0.1"},
        {"symbol": "BTCUSDT", "orderId": "sl_2", "type": "LIMIT", "stopPrice": "90", "reduceOnly": True, "side": "SELL", "quantity": "0.1"},
    ]
