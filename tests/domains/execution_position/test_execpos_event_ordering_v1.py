...
def test_ack_portfolio_fill_order(fsm_harness):
    ...
    ack_msg = Message(op="EVT", verb="ORDER_ACK", src="adapter", dst="exec", pld=ack_pld)
    ...
    port_msg = Message(op="EVT", verb="PORTFOLIO_STATE_UPDATED", src="adapter", dst="exec", pld=port_pld)
    fsm._on_portfolio_state_updated(port_msg)
    assert fsm._latest_portfolio_state["total_equity"] == "10000"
    
    # Simulate FILL
    fill_pld = {"orderId": "ord_1", "symbol": "BTCUSDT", "quantity": "0.1", "rid": "r1"}
    fill_msg = Message(op="EVT", verb="ORDER_FILL", src="adapter", dst="exec", pld=fill_pld)
    fsm._on_order_fill(fill_msg)
    assert "fill_ord_1_BTCUSDT" in fsm._processed_events
...
def test_fill_before_ack_out_of_order(fsm_harness):
    ...
    fill_pld = {"orderId": "ord_2", "symbol": "BTCUSDT", "quantity": "0.1", "rid": "r2"}
    fill_msg = Message(op="EVT", verb="ORDER_FILL", src="adapter", dst="exec", pld=fill_pld)
    fsm._on_order_fill(fill_msg)
    assert "fill_ord_2_BTCUSDT" in fsm._processed_events
...
def test_portfolio_lagging_after_open(fsm_harness):
    ...
    fsm._on_portfolio_state_updated(Message(op="EVT", verb="P", src="adapter", dst="exec", pld=port_pld))
    ...
    fill_pld = {"orderId": "ord_lag", "symbol": "BTCUSDT", "quantity": "0.5", "rid": "rlag"}
    fill_msg = Message(op="EVT", verb="ORDER_FILL", src="adapter", dst="exec", pld=fill_pld)
    fsm._on_order_fill(fill_msg)
    
    # Check if exposure guard now has the reservation
    assert any("ord_lag" in k for k in fsm.exposure_guard.state.postfill_reservations.keys())

def test_duplicate_fill_idempotency(fsm_harness):
    """Duplicate FILL event (same event id / orderId)"""
    fsm, bus, cfg = fsm_harness
    
    fill_pld = {"orderId": "ord_dup", "symbol": "BTCUSDT", "quantity": "0.1", "rid": "rdup"}
    fill_msg = Message(op="EVT", verb="ORDER_FILL", src="adapter", dst="exec", pld=fill_pld)
    ...
    assert "fill_ord_dup_BTCUSDT" in fsm._processed_events
