...
def test_build_order_params_market_and_limit():
    cfg = {"trading_env": "test", "trading": {"execution": {"open_order_type": "MARKET", "order_params": {}}}, "binance_ro_api_key": "k", "binance_ro_api_secret": "s"}
    ...
    cfg_l = {"trading_env": "test", "trading": {"execution": {"open_order_type": "LIMIT", "order_params": {"LIMIT": {"timeInForce": "GTC"}}}}, "binance_ro_api_key": "k", "binance_ro_api_secret": "s"}
...
def test_generate_signature_matches_manual():
    ...
    expected = hmac.new(secret.encode('utf-8'), qs.encode('utf-8'), hashlib.sha256).hexdigest()
    assert sig == expected


@pytest.mark.asyncio
async def test_place_order_shadow_mode():
    cfg = {"binance_ro_api_secret": "s", "binance_ro_api_key": "k", "trading_env": "test"}
    adapter = BinanceExecutionAdapter(fsm=None, config=cfg, shadow_mode=True)
    msg = Message(op="DEC", verb="OPEN", src="t", dst="b", rid="r1", pld={"symbol": "ETHUSDT", "qty": "1"})
...
async def test_place_order_shadow_mode_success(adapter_shadow):
    dec_msg = Message(op="DEC", verb="OPEN", src="test", dst="test", pld={"symbol": "BTCUSDT", "side": "BUY", "qty": "1"})
