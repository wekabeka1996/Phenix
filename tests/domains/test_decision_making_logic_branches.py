...
def test_strong_long_signal_emits_trade(mock_fsm, full_valid_config):
    ...
    features_msg = Message(op="EVT", verb="FEATURES_CALCULATED", src="test", dst="test", pld={"symbol": "BTCUSDT", "features": features})
    risk_msg = Message(op="EVT", verb="RISK_ASSESSMENT_COMPLETED", src="test", dst="test", pld={"symbol": "BTCUSDT", "risk_parameters": {"is_trading_allowed": True}})
    portfolio_msg = Message(op="EVT", verb="PORTFOLIO_STATE_UPDATED", src="test", dst="test", pld={"equity": "50000"})
...
def test_position_size_below_minimum_rejected(mock_fsm, full_valid_config, caplog):
    ...
    config_low_liq['trading']['decision']['position_sizing']['liquidity_based_cap_usd'] = 5
