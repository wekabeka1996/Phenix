...
def make_fill_msg(rid="r1", qty=1, price=100.0, side="BUY"):
    return Message(op="EVT", verb="FILL", src="t", dst="mgr", rid=rid, pld={"qty": qty, "price": price, "side": side})
...
def test_place_brackets_and_on_bracket_placed():
    ...
    sl_msg = Message(op="EVT", verb="ORDER_UPDATED", src="ex", dst="mgr", rid="b1", pld={"clientOrderId": "b1_1_sl", "orderId": "o_sl"})
    tp_msg = Message(op="EVT", verb="ORDER_UPDATED", src="ex", dst="mgr", rid="b1", pld={"clientOrderId": "b1_1_tp", "orderId": "o_tp"})
...
def test_trailing_activation_and_adjust():
    cfg = {"trailing": {"enable": True, "activation_profit_atr_k": 1.0, "cooldown_sec": 0, "step_bps": 10}}
    ...
    msg = Message(op="UPD", verb="MARKET_DATA", src="md", dst="mgr", rid="t1", pld={"mark_price": 110})
