...
def _make_config(symbol: str = "BNBUSDT"):
    ...
    md_amr_cfg = SimpleNamespace(
        ...
        reconciliation=SimpleNamespace(enabled=True, interval_sec=300, drift_tolerance=1e-6),
        concentration_guard=SimpleNamespace(enabled=False, max_simultaneous_entries_per_bar=2),
...
def test_md_amr_on_process_strategy_preserves_anchor_surface_with_c3() -> None:
    ...
    handler._strategies = {
        "BNBUSDT": SimpleNamespace(
            on_bar=lambda **kwargs: captured.append(kwargs) or {"status": "NOOP"}
        )
    }
