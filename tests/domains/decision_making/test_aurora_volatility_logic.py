...
class TestVolatilityEntryLogic:
    ...
    def handler(self):
        ...
        handler._symbol_states["TESTUSDT"].last_regime_heartbeat_ms = int(handler.monotonic_fn() * 1000)
    ...
    def test_long_entry_offset_below_anchor(self, handler):
        ...
        # 100 - (2 * 0.6) = 98.8
        assert entry_price == Decimal("98.8"), f"Expected 98.8, got {entry_price}"
    ...
    def test_short_entry_offset_above_anchor(self, handler):
        ...
        # 100 + (2 * 0.6) = 101.2
        assert entry_price == Decimal("101.2"), f"Expected 101.2, got {entry_price}"
    ...
    def test_missing_atr_aborts_signal(self, handler):
        ...
        assert len(signal_calls) == 0, "Signal should NOT be emitted when ATR missing"
        ...
        assert len(blocked_calls) >= 1, "DECISION_BLOCKED should be emitted on ATR missing"
    ...
    def test_unknown_regime_uses_default(self, handler):
        ...
        assert entry_price == Decimal("99.6"), f"Expected 99.6 (DEFAULT mult=0.2), got {entry_price}"
