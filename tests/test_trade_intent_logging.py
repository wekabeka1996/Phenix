...
def test_aurora_trade_intents_formatted_log_file():
    ...
    test_trade_intent = {
        ...
        "risk_budget": {"trade_cvar95_max_bps": 100.0, "session_cvar95_max_bps": 200.0},
        "size": {"kelly_fraction": 0.6, "notional_cap_usd": 0.0},
        "valid_for_ms": 30000,
        "why": [
    ...
    assert "🚀 NEW TRADE INTENT PROPOSED! 🚀" in content, "Should contain emoji header"

    # 5. Verify JSON structure with proper indentation
    assert '"instrument": "ETHUSDT"' in content, "Should contain instrument field"

    assert '"side": "buy"' in content, "Should contain side field"

    assert '"p": 0.8' in content, "Should contain probability p"

    assert '"payoff_ratio_r": 2.0' in content, "Should contain payoff ratio"
    ...
    assert '"why": [' in content, "Should contain why array"

    assert "Decision based on signal_score=" in content, (
        "Should contain signal score in why"
    )

    assert "Features: obi=" in content, "Should contain features in why"

    assert "Risk approved: kelly=" in content, "Should contain risk approval in why"
