...
class BlockRedisImportFinder(MetaPathFinder):
    ...
    def find_spec(self, fullname: str, path: Optional[list[str]], target: Optional[ModuleType] = None) -> None:
...
def test_import_guard_sets_false(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    modules_to_remove = [k for k in sys.modules if k == "redis" or k.startswith("redis.")]
    ...
    try:
        ...
        redis_store_path = os.path.join(
            ...
            "..", "..", "vfoundation", "vfoundation", "core", "idempotency", "backends", "redis_store.py"
        )
        ...
        assert module.REDIS_AVAILABLE is False, "REDIS_AVAILABLE should be False when redis import blocked"
