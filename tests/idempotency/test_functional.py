...
@pytest.fixture
def store(redis_url: str, worker_id: str, monkeypatch: pytest.MonkeyPatch) -> Generator[SimpleRedisIdempotencyStore, None, None]:
...
def test_idemp_reserve_duplicate_same(store: SimpleRedisIdempotencyStore, worker_id: str) -> None:
...
def test_idemp_reserve_duplicate_conflict(store: SimpleRedisIdempotencyStore, worker_id: str) -> None:
...
def test_idemp_confirm_ok(store: SimpleRedisIdempotencyStore, worker_id: str) -> None:
    ...
    # Confirm
    confirm_result = store.confirm(key, final_status="ORDER_PLACED", meta={"eid": "12345"})
...
def test_idemp_missing_confirm_release(store: SimpleRedisIdempotencyStore, worker_id: str) -> None:
