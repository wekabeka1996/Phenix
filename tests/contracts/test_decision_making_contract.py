...
def test_emitted_trade_intent_conforms_to_schema(decision_domain_for_contract_test):
    ...
    risk_event = Message(op="EVT", verb="RISK_ASSESSMENT_COMPLETED", src="risk", dst="decision", pld=risk_payload)
    portfolio_event = Message(op="EVT", verb="PORTFOLIO_STATE_UPDATED", src="portfolio", dst="decision", pld=portfolio_payload)
