...
def test_abstract_exchange_adapter_methods():
    ...
    class MockAdapter(AbstractExchangeAdapter):
        ...
        async def cancel_order(self, *a, **k): await super().cancel_order(*a, **k)
        async def create_order(self, *a, **k): await super().create_order(*a, **k)
        async def get_account_balance(self, *a, **k): await super().get_account_balance(*a, **k)
        async def get_exchange_info(self, *a, **k): await super().get_exchange_info(*a, **k)
        async def get_last_price(self, *a, **k): await super().get_last_price(*a, **k)
        async def get_mark_price(self, *a, **k): await super().get_mark_price(*a, **k)
        async def get_open_orders(self, *a, **k): await super().get_open_orders(*a, **k)
        async def get_open_positions(self, *a, **k): await super().get_open_positions(*a, **k)
        def quantize_quantity(self, *a, **k): super().quantize_quantity(*a, **k)
...
def test_execution_adapter_base_methods():
    ...
    class MockExecAdapter(ExecutionAdapter):
        ...
        def _cancel_impl(self, order_id, client_order_id): return super()._cancel_impl(order_id, client_order_id)
