...
class TestP1FallbackConfigSchema:
    """P1.1: Verify FallbackConfig Pydantic model has required fields."""

    def test_fallback_config_schema_basic(self):
        """FallbackConfig model must have policy, risk_reduction_pct, backoff_ms."""
        cfg = FallbackConfig(
            policy="fail_closed",
        ...
        assert cfg.risk_reduction_pct == Decimal("0.5")
        assert cfg.backoff_ms == [100, 200, 500]

    def test_fallback_config_reduce_exposure_policy(self):
        """FallbackConfig accepts 'reduce_exposure' policy."""
        cfg = FallbackConfig(
            policy="reduce_exposure",
            risk_reduction_pct=Decimal("0.3"),
            backoff_ms=[50, 100]
        )
        assert cfg.policy == "reduce_exposure"
...
class TestP1ExposureGuardFallbackConfig:
    ...
    def test_exposure_guard_loads_fallback_from_config(self):
        ...
        assert guard.fallback_config["risk_reduction_pct"] == Decimal("0.5")
