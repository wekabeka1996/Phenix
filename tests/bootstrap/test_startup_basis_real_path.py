...
class _TransientBackfillAdapter:
    ...
    async def get_klines(self, symbol: str, interval: str, limit: int):
        ...
        self.calls[key] += 1
        if self.calls[key] == 1:
            raise ConnectionError(f"startup transient failure for {symbol}/{interval}")
...
def test_live_config_hydration_plan_matches_runtime_contract() -> None:
    ...
    assert plan.plans["md_amr:BNBUSDT"].requirement.basis_required_bars == 301
    assert plan.plans["md_amr:XRPUSDT"].requirement.basis_required_bars == 301
