...
def create_app_with_env(trading_env=None):
    ...
    with patch.dict(os.environ, env_patch, clear=bool(not trading_env)):
        ...
        ) if k.startswith('apps.reference.api')]
...
def test_debug_api_is_enabled_in_development():
    """
    Verify that when TRADING_ENV is 'development', the debug API is available (200 on /health).
    """
    client = create_app_with_env("development")
    # ✅ Перевіримо /health endpoint (завжди доступний), замість неіснуючого RID
...
def test_default_env_is_development():
    """
    Verify that when TRADING_ENV is not set, the system defaults to development mode (200 on /health).
    """
