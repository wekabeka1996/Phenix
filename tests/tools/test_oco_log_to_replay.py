...
def test_e2e_file_processing(tmp_path):
    ...
    # Message must contain "BRACKET_EVAL_SNAPSHOT" for parser to process it
    lines = [
        json.dumps({"timestamp": "2025-01-01T10:00:00Z", "message": "BRACKET_EVAL_SNAPSHOT symbol=BTCUSDT",
                   "extra": {"snapshot": json.dumps(snapshot1)}}),
        json.dumps({"timestamp": "2025-01-01T10:00:01Z", "message": "BRACKET_EVAL_SNAPSHOT symbol=ETHUSDT",
                   "extra": {"snapshot": json.dumps(snapshot2)}}),
        json.dumps({"timestamp": "2025-01-01T10:00:02Z",
                   "message": "Log 3"})  # No snapshot
    ]

    input_file.write_text("\n".join(lines), encoding="utf-8")

    # Run script via CLI args simulation
    test_args = ["oco_log_to_replay.py", "--runtime-logs",
                 str(input_file), "--out", str(output_file)]
