...
class TestSignalWeightsConfig:
    ...
    def test_signal_weights_in_config(self):
        ...
        with open(trading_yaml, 'r') as f:
            content = f.read()
            assert "signal_weights:" in content, "signal_weights not found in trading.yaml"
...
class TestSignalCalculation:
    ...
    @pytest.mark.parametrize("features,expected,desc", [
        (
...
class TestFeatureFormulas:
    ...
    def test_feature_ranges(self):
        ...
        assert -0.5 <= realistic_features["delta_price"] <= 0.5, "delta_price out of range"
...
class TestWeightSumValidation:
    ...
    def test_weights_sum_close_to_one(self):
        ...
        assert abs(weight_sum - 1.0) < 0.0001, \
            f"Weights don't sum to 1.0: {weight_sum}"
