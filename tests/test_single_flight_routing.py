...
def test_single_flight_concurrent_requests():
    ...
    def slow_handler(msg: Message) -> Message:
        ...
        return Message(
            op="EVT", verb="SUCCESS", src="handler", dst=msg.src, 
            rid=msg.rid, why="processed", pld={"result": "ok"}
        )
    ...
    def make_request(idx: int):
        msg = Message(
            op="DEC", verb="TEST", src="client", dst="handler",
            rid=f"rid-{idx}", idempotent_key="test-key-123",
            why="concurrent test"
        )
        _sign_message(msg)
    ...
    assert execution_count[0] == 1, f"Handler executed {execution_count[0]} times, expected 1"
...
def test_single_flight_dedup_after_completion():
    ...
    def handler(msg: Message) -> Message:
        ...
        return Message(
            op="EVT", verb="SUCCESS", src="handler", dst=msg.src,
            rid=msg.rid, why="processed", pld={"result": execution_count[0]}
        )
    ...
    msg1 = Message(
        op="DEC", verb="TEST", src="client", dst="handler",
        rid="rid-1", idempotent_key="dedup-key",
        why="first request"
    )
    _sign_message(msg1)
    ...
    msg2 = Message(
        op="DEC", verb="TEST", src="client", dst="handler",
        rid="rid-2", idempotent_key="dedup-key",
        why="second request"
    )
    _sign_message(msg2)
...
def test_single_flight_metrics():
    ...
    msg1 = Message(
        op="DEC", verb="TEST", src="client", dst="handler",
        rid="rid-1", idempotent_key="metrics-key",
        why="metrics test"
    )
    _sign_message(msg1)
    router.route(msg1)
    ...
    msg2 = Message(
        op="DEC", verb="TEST", src="client", dst="handler",
        rid="rid-2", idempotent_key="metrics-key",
        why="metrics test 2"
    )
    _sign_message(msg2)
    router.route(msg2)
...
def test_single_flight_different_keys():
    ...
    def handler(msg: Message) -> Message:
        ...
        return Message(
            op="EVT", verb="SUCCESS", src="handler", dst=msg.src,
            rid=msg.rid, why="ok", pld={"execution": execution_count[0]}
        )
    ...
    msg1 = Message(
        op="DEC", verb="TEST", src="client", dst="handler",
        rid="rid-1", idempotent_key="key-A", why="test A"
    )
    _sign_message(msg1)
    msg2 = Message(
        op="DEC", verb="TEST", src="client", dst="handler",
        rid="rid-2", idempotent_key="key-B", why="test B"
    )
    _sign_message(msg2)
