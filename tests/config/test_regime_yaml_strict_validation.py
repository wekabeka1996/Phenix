...
class TestRegimeYamlStrictValidation:
    ...
    def test_valid_regime_yaml_loads_successfully(self, tmp_path):
        ...
        # Start from canonical SSOT because the root regime contract now carries
        # more required fields than the old minimal fixture covered.
        regime_yaml = yaml.safe_load(
            (config_dir / "regime.yaml").read_text(encoding="utf-8")
        )
        regime_yaml["basis_tf_sec"] = 300
        regime_yaml["uncertain_cutoff"] = 0.35
        regime_yaml["liveness_factor"] = 3
        regime_yaml["models"]["sma_trend"]["sma_short_period"] = 10
        regime_yaml["models"]["sma_trend"]["sma_long_period"] = 50
        regime_yaml["models"]["mean_reversion"]["threshold"] = 0.005
