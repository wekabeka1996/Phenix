...
def test_wal_concurrent_appends_integrity():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        assert all(results), f"Some threads failed: {errors[:5]}"  # Show first 5 errors if any
        ...
        assert lock_metrics["lock_contention"] > 0, "Expected some lock contention with 100 threads"
...
def test_wal_high_throughput():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        # Verify chain integrity
        assert verify_chain(records), "Chain integrity failed after high-throughput writes"
