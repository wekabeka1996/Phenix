...
base_path = os.path.join(os.path.dirname(__file__), "..", "..", "apps", "reference", "domains", "execution_position")
exec_adapter_path = os.path.join(base_path, "execution_adapter.py")
spec_exec = importlib.util.spec_from_file_location("apps.reference.domains.execution_position.execution_adapter", exec_adapter_path)
...
spec_bin = importlib.util.spec_from_file_location("apps.reference.domains.execution_position.binance_execution_adapter", binance_path)
...
def test_build_order_params_market_and_limit():
    ...
    params_m = BinanceExecutionAdapter._build_order_params.__get__(dummy_m, BinanceExecutionAdapter)(pld)
    assert params_m["type"] == "MARKET"
    assert params_m["quantity"] == "0.2"

    cfg_limit = {"trading": {"execution": {"open_order_type": "LIMIT", "order_params": {"LIMIT": {"timeInForce": "GTC"}}}}}
    dummy_l = types.SimpleNamespace(config=cfg_limit)
    pld2 = {"symbol": "BTCUSDT", "side": "SELL", "qty": "0.5", "price": "40000"}
    params_l = BinanceExecutionAdapter._build_order_params.__get__(dummy_l, BinanceExecutionAdapter)(pld2)
