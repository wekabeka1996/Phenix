...
async def test_bridge_injects_tick_to_marketdata(monkeypatch):
    ...
    class MockAdapter:
        async def get_book_ticker(self, symbol):
            return {"bidPrice": "100.0", "bidQty": "1.0", "askPrice": "100.1", "askQty": "1.0"}
    ...
    tick = {"symbol": "BTCUSDT", "bid": "100.0", "ask": "100.1", "trades": {"buy": 1, "sell": 1}, "ts": 42}
    msg = Message(op="EVT", verb="MARKET_TICK_RECEIVED", intent="OBSERVATION",
                  src="bridge", dst="any", rid="t1", pld=tick, why="test_bridge")
