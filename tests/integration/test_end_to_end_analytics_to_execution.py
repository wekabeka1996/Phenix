...
def test_end_to_end_analytics_to_execution_bridge(full_config):
    ...
    execution_position_mock.handle = MagicMock(
    ...
    def on_trade_intent_proposed(event):
        ...
        if execution_position_mock is not None:
            ...
            if result:
                fsm.logger.info(
                    f"BRIDGE: Execution FSM processed, result: {result.op}:{result.verb}"
                )
    ...
    assert execution_position_mock.handle.called, (
        "Execution FSM should receive CMD:OPEN"
    )
    ...
    assert called_command.dst == "execution_position", (
        "Destination should be execution_position"
    )
    ...
    assert called_command.parent_span_id == "parent-span-789", (
        "Should link to parent event span"
    )
