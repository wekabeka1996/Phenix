...
class FSMCore:
    ...
    def emit(self, event_name: str, payload=None, why=None, **kwargs) -> None:
        ...
        if event_name in self.listeners:
            event = Message(op=kwargs.get('op', 'EVT'), verb=event_name, pld=payload, why=why)
...
def test_full_flow_from_decision_to_execution_command(full_config):
    ...
    def on_trade_intent_proposed(event):
        ...
        command_payload = {
            ...
            "idempotent_key": event.pld.get("idempotent_key"),  # Pass through for deduplication
        }
    ...
    fsm_core.emit("TRADE_INTENT_PROPOSED", payload=trade_intent_payload, why="Strong buy signal detected")
    ...
    assert called_command.verb == "OPEN", f"Expected verb='OPEN', got '{called_command.verb}'"
    ...
    assert "idempotent_key" in called_command.pld, "idempotent_key should be passed through from intent"
    assert isinstance(called_command.pld["idempotent_key"], str), "idempotent_key should be string"
    assert len(called_command.pld["idempotent_key"]) == 32, "idempotent_key should be 32-char hash"
    
    # Validate XAI chain preservation
    assert called_command.why.startswith("Decision based on"), "Why should preserve first element from decision why chain"
    ...
    assert hasattr(called_command, 'parent_span_id'), "Command should have parent_span_id for tracing"
    ...
    assert any("BRIDGE: Received TRADE_INTENT_PROPOSED" in msg for msg in log_messages), "Should log event reception"
    assert any("BRIDGE: Dispatched CMD:OPEN" in msg for msg in log_messages), "Should log command dispatch"
