...
def mock_config():
    ...
    return {
        "trading": {
            ...
            "tca_prefs": {"maker_preference": "neutral", "max_slippage_bps": 5, "max_latency_ms": 45},
            "risk_budgets": {"trade_cvar95_max_bps": 100, "session_cvar95_max_bps": 500},
            "instruments": {
                "ETHUSDT": {"min_qty": "0.001", "step_size": "0.001", "tick_size": "0.01"}
            }
        }
    }
...
class TestDecimalPrecisionPreservation:
    ...
    """
    def test_trade_intent_payload_uses_strings_not_floats(self, decision_making_domain, mock_config):
        """
        ...
        features_payload = {
            "symbol": "ETHUSDT", "ts": 1672531200,
            "features": {"obi": 0.8, "tfi": 0.8, "absorption": 0.5, "price": "4000.987654321"}
        }
        risk_payload = {"symbol": "ETHUSDT", "risk_parameters": {"is_trading_allowed": True, "leverage": 10}}
        portfolio_payload = {"equity": "10000", "positions": {}}
        
        features_event = Message(op="EVT", verb="FEATURES_CALCULATED", src="feature_engineering", dst="decision_making", pld=features_payload)
        risk_event = Message(op="EVT", verb="RISK_ASSESSMENT_COMPLETED", src="risk_management", dst="decision_making", pld=risk_payload)
        portfolio_event = Message(op="EVT", verb="PORTFOLIO_STATE_UPDATED", src="position_tracking", dst="decision_making", pld=portfolio_payload)
