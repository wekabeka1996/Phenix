...
class TestFailClosedPricePattern:
    ...
    def test_decision_proceeds_when_price_available_in_features(self):
        ...
        emitted_events = []
        mock_fsm_core.emit.side_effect = lambda name, payload, why: emitted_events.append(name)

        config = {
            'trading': {
                'instruments': { 'ETHUSDT': { 'lot_step': 0.001, 'tick_size': 0.01, 'min_qty': 0.001, 'step_size': '0.001' }},
                'decision': {
                    'signal_weights': {'obi': 0.3, 'tfi': 0.4, 'absorption': 0.3},
                    'probability_bounds': {'base': 0.5, 'max_prob': 0.8},
                    'payoff_ratio_r': 2.0,
                    'position_sizing': {
                        'kelly_conservative_factor': 0.1, 'kelly_alpha': 0.5,
                        'min_position_size_usd': 10.0, 'max_position_size_usd': 1000.0,
                        'default_notional_cap_usd': 1000.0, 'liquidity_based_cap_usd': 10000.0
                    }
                },
        ...
        features_msg = Message(op="EVT", verb="FEATURES_CALCULATED", src="test", dst="test", pld={
            'symbol': 'ETHUSDT', 'ts': 1640995200000,
            'features': { 'price': '3850.50', 'obi': '0.5', 'tfi': '0.3', 'absorption': '1.2', 'delta_price': '10.0' }
        })
        risk_msg = Message(op="EVT", verb="RISK_ASSESSMENT_COMPLETED", src="test", dst="test", pld={
            'symbol': 'ETHUSDT', 'ts': '2025-01-01T12:00:00Z',
            'risk_parameters': { 'is_trading_allowed': True }
        })
        portfolio_msg = Message(op="EVT", verb="PORTFOLIO_STATE_UPDATED", src="test", dst="test", pld={'equity': '10000.0'})
        ...
        assert len(emitted_events) == 1, "Expected TRADE_INTENT_PROPOSED when price available"
    ...
    def test_no_hardcoded_fallback_price_in_code(self):
        ...
        assert "fallback price" not in source.lower() or "fail-closed" in source.lower(), \
