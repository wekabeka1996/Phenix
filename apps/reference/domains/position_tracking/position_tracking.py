...
class PositionTracking:
    ...
    def on_account_update(self, event: Message) -> None:
        ...
        try:
            ...
            if wal_hash is None:
                ...
                return
            self.logger.debug(f"WAL: Appended ACCOUNT_UPDATE_RECEIVED to WAL with hash={wal_hash[:8]}...")
        except Exception as e:
            # Any WAL write failure is critical - we cannot process without durability guarantee
            self.logger.critical(
        ...
        if self._initial_balance is None:
            ...
            self.logger.info(f"[STATE_SYNC] Initial balance set: ${self._initial_balance}")
        else:
        ...
        for pos in account_positions:
            symbol = pos['symbol']
            quantity = decimal.Decimal(str(pos.get('positionAmt', 0)))
            if abs(quantity) > decimal.Decimal('1e-9'):  # Only track non-zero positions
                self._positions[symbol] = {
                    "quantity": quantity,
                    "avg_price": decimal.Decimal(str(pos.get('entryPrice', 0))),
        ...
        self.logger.info(f"Updated portfolio from account: equity={self._equity}, positions={len(self._positions)}")
    ...
    def _get_positions_snapshot(self) -> List[Dict[str, Any]]:
        ...
        positions = []
        for symbol, position in self._positions.items():
            if abs(position["quantity"]) > decimal.Decimal('1e-9'):  # Only include non-zero positions
                positions.append({
                    "symbol": symbol,
                    "net_position": str(position["quantity"]),  # Preserve Decimal precision as string
                    "avg_entry_price": str(position["avg_price"]),  # Preserve Decimal precision as string
