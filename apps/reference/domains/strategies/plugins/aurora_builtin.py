...
class _AuroraHandlerWrapper:
    ...
    def register(self) -> None:
        ...
        # T2B-03: Keep FEATURES_CALCULATED for data-only (warmup caching), NOT as trigger
    ...
    def _on_features_data_only(self, event: Any) -> None:
        """T2B-03: Data-only handler for FEATURES_CALCULATED (warmup caching, NO decision trigger)."""
        pld = event.pld if hasattr(event, "pld") else event
