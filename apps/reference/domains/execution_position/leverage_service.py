...
class LeverageService:
    ...
    async def verify(
        ...
        try:
            ...
            LOG.error(f"LEVERAGE_SERVICE: get_current_leverage failed for {symbol}: {e}")
            return VerifyResult(
                ok=False,
        ...
        try:
            ...
            LOG.error(f"LEVERAGE_SERVICE: get_margin_mode failed for {symbol}: {e}")
            return VerifyResult(
                ok=False,
    ...
    async def set_and_verify(
        ...
        if not skip_set:
            ...
            try:
                ...
                LOG.error(f"LEVERAGE_SERVICE: set_margin_mode failed for {symbol}: {e}")
                return VerifyResult(
                    ok=False,
                    actual_leverage=None,
                    actual_margin_mode=None,
            ...
            try:
                ...
                LOG.error(f"LEVERAGE_SERVICE: set_leverage failed for {symbol}: {e}")
                return VerifyResult(
                    ok=False,
                    actual_leverage=None,
