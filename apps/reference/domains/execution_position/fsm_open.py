...
class OpenFlowFSM:
    ...
    def _cleanup_idempotency_store(self):
        ...
        now = time.time()
        expired_keys = [
            key for key, timestamp in self.idempotency_store.items()
            if now - timestamp > self.idempotency_window_sec
        ]
        for key in expired_keys:
            del self.idempotency_store[key]
        if expired_keys:
            self.logger.info(f"IDEMPOTENCY: Cleaned up {len(expired_keys)} expired keys.")
        ...
        if msg.op == "CMD" and msg.verb == "OPEN":
            ...
            if idempotent_key:
                if idempotent_key in self.idempotency_store:
                    self.logger.warning(f"IDEMPOTENCY_REJECT: Duplicate CMD:OPEN received with key {idempotent_key}, rid={msg.rid}")
            ...
            try:
                ...
                price_ref = Decimal(str(pld.get("price_ref"))) if pld.get("price_ref") is not None else None
                ...
                if qty_rounded != qty_dec:
                    self.logger.warning(f"GUARD_ADJUST: Quantity rounded down - original={qty_dec}, rounded={qty_rounded}, step={step_size}, rid={msg.rid}")
                ...
                if order_type == "LIMIT":
                    ...
                    if price_rounded != price_dec:
                        self.logger.warning(f"GUARD_ADJUST: Price rounded to tick - original={price_dec}, rounded={price_rounded}, tick={tick_size}, rid={msg.rid}")
                ...
                if "idempotent_key" in msg.pld:
                    ...
                    self.logger.info(f"IDEMPOTENCY: Passing key {msg.pld['idempotent_key']} to DEC:OPEN")

                self.logger.info(f"GUARD_PASSED: All guards OK - symbol={symbol}, side={side}, qty={qty_dec}, price={price_dec}, order_type={order_type}, rid={msg.rid}")
                ...
                self.logger.exception(f"FSM_EXCEPTION: Unexpected error processing CMD:OPEN - rid={msg.rid}, error={str(e)}")
                return Message(
                    op="ERR",
                    verb="OPEN",
                    src=msg.dst,
