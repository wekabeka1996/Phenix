"""
Close/cancel order execution for execution_position domain.

Extracted from ExecPosFSM._execute_decision (Phase 14A decomposition).
Handles CANCEL_ORDER, CLOSE/CLOSE_POSITION, and PLACE_ORDER verbs.
"""
...
class CloseExecutor:
    ...
    async def execute_close(self, decision: "Message") -> None:
        """Handle CLOSE/CLOSE_POSITION verb - full position close with bracket cleanup."""
        pld = decision.pld or {}
        symbol = pld.get("symbol")
        if not symbol:
        ...
        if manage:
            ...
            LOG.info(f"🔒 [PHASE A2] Set closing flag for {symbol} to prevent bracket race")
            ...
            for (bracket_type, oid), result in zip(bracket_order_ids, results):
                if isinstance(result, Exception):
                    if self._fsm._is_unknown_order_error(result):
                        LOG.info(f"ℹ️ {bracket_type} bracket {oid} already absent (-2011) for {symbol}")
                        order_logger.write({
                        ...
                        LOG.warning(f"❌ Failed to cancel {bracket_type} bracket {oid} for {symbol}: {result}")
                        order_logger.write({
                            "rid": decision.rid or "manual-close",
                    ...
                    if self._fsm._is_cancel_success_response(result):
                        LOG.info(f"✅ Cancelled {bracket_type} bracket {oid} for {symbol}")
                        order_logger.write({
                            "rid": decision.rid or "manual-close",
                            "event_type": "ORDER_CANCELLED", "symbol": symbol,
                            "order_id": oid, "bracket_type": bracket_type,
                        ...
                        LOG.warning(f"❌ Cancel rejected for {bracket_type} bracket {oid}: status={cancel_status}")
        ...
        LOG.info(f"Close executed for {symbol}: side={close_side} qty={close_qty}")
        self._fsm._symbol_brackets.pop(symbol, None)
        ...
        try:
            ...
            for o in open_orders_list:
                ...
                reduce_only = str(o["reduceOnly"] if "reduceOnly" in o else "").lower() == "true"
                close_pos = str(o["closePosition"] if "closePosition" in o else "").lower() == "true"
                if otype in ("STOP_MARKET", "TAKE_PROFIT_MARKET", "LIMIT") and (reduce_only or close_pos):
                    oid = o.get("orderId")
                    cancel_tasks.append((otype, oid, self._fsm._cancel_order(symbol, oid)))
            ...
            if cancel_tasks:
                ...
                for (otype, oid, _), result in zip(cancel_tasks, results):
                    if isinstance(result, Exception):
                        if self._fsm._is_unknown_order_error(result):
                            LOG.info(f"ℹ️ [DEC:CLOSE RECONCILE] {otype} {oid} already gone for {symbol} (-2011)")
                        else:
                            LOG.warning(f"❌ [DEC:CLOSE RECONCILE] Failed to cancel {otype} {oid} for {symbol}: {result}")
                            self._fsm._orphan_metrics["errors"] += 1
                    else:
                        if self._fsm._is_cancel_success_response(result):
                            LOG.info(f"✅ [DEC:CLOSE RECONCILE] Cancelled {otype} {oid} for {symbol}")
                            self._fsm._orphan_metrics["reconcile_cancelled"] += 1
                        else:
                            status = self._fsm._cancel_status_str(result)
                            LOG.warning(f"❌ [DEC:CLOSE RECONCILE] Cancel response unexpected for {otype} {oid} (status={status})")
                            self._fsm._orphan_metrics["errors"] += 1
                ...
                LOG.info(f"✅ [DEC:CLOSE RECONCILE] No orphaned brackets found for {symbol}")
        except Exception as e:
            LOG.warning(f"⚠️ [DEC:CLOSE RECONCILE] Error during cleanup for {symbol}: {e}")
            self._fsm._orphan_metrics["errors"] += 1
        ...
        if manage:
            ...
            LOG.info(f"🔓 [PHASE A2] Cleared closing flag for {symbol} - CLOSE complete")
    ...
    async def execute_place_order(self, decision: "Message") -> None:
        """Handle generic PLACE_ORDER verb (e.g. from ManageFlowFSM for brackets)."""
        ...
        try:
            ...
            # ORDER_INDEX: correlate bracket/aux orders
            try:
            ...
            if client_id and resp:
                ...
                if "_sl" in client_id:
                    self._fsm._symbol_brackets.setdefault(symbol, {})["sl_order_id"] = order_id
                elif "_tp" in client_id:
                    self._fsm._symbol_brackets.setdefault(symbol, {})["tp_order_id"] = order_id
