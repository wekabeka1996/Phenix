...
from .soft_clip import (
    SoftClipEngine,
    SoftLimitConfig,
    load_soft_limit_config,
)
from apps.reference.domain_config import DomainConfigResolver as _DomainConfigResolver
...
def _decimal_payload(value: Decimal) -> str:
    """Serialize financial Decimal values without passing through float()."""
    return format(value, "f")


def _decimal_2dp(value: Decimal) -> str:
    """Format Decimal values for logs without float-based rounding."""
...
class ExposureGuard:
    ...
    def __init__(self, fsm_core: Any, config: AuroraConfig):
        ...
        self.logger = logging.getLogger(
            f"apps.reference.domains.execution_position.exposure_guard.{self.__class__.__name__}"
        )

        # =====================================================================
        # PHASE 2: Use DomainConfigResolver for clean config access
        # =====================================================================
        resolver_factory = globals().get("DomainConfigResolver", _DomainConfigResolver)
        ...
        self.max_equity_utilization_pct = self._to_dec(
            eg_config.max_equity_utilization_pct, path="domains.execution_position.exposure_guard.max_equity_utilization_pct") / Decimal("100")
        self.max_portfolio_fraction = self._to_dec(
            eg_config.max_portfolio_fraction, path="domains.execution_position.exposure_guard.max_portfolio_fraction")
        self.max_long_utilization_pct = self._to_dec(
            eg_config.max_long_utilization_pct, path="domains.execution_position.exposure_guard.max_long_utilization_pct") / Decimal("100")
        self.max_short_utilization_pct = self._to_dec(
            eg_config.max_short_utilization_pct, path="domains.execution_position.exposure_guard.max_short_utilization_pct") / Decimal("100")
        self.max_concentration_pct = self._to_dec(
            eg_config.max_concentration_pct, path="domains.execution_position.exposure_guard.max_concentration_pct") / Decimal("100")

        # Directional ratio (legacy support)
        # Fail-closed: ExposureGuardConfig is strict and requires this field.
        self.max_directional_ratio = self._to_dec(
            eg_config.max_directional_ratio, path="domains.execution_position.exposure_guard.max_directional_ratio")
        ...
        self.count_pending_orders = bool(
            execution_cfg.exposure.count_pending_orders)
        self.exclude_reduce_only = bool(
            execution_cfg.exposure.exclude_reduce_only)
    ...
    def _load_soft_limit_config(self) -> SoftLimitConfig:
        ...
        try:
            ...
            raise ConfigContractError(
                path="trading.risk.soft_limits", why=str(e)) from e
    ...
    def _to_dec(self, val: Any, *, path: str, default_on_error: Optional[Decimal] = None) -> Decimal:
        ...
        if val is None:
            if default_on_error is not None:
                self.logger.error(
                    "CONFIG_DECIMAL_MISSING: %s is None; defaulting to %s", path, default_on_error)
                return default_on_error
            self.logger.error("CONFIG_DECIMAL_MISSING: %s is None", path)
            raise ConfigContractError(
                path=path, why="Missing required decimal value")
        try:
            return Decimal(str(val))
        except (InvalidOperation, ValueError, TypeError) as e:
            if default_on_error is not None:
                self.logger.error(
                    "CONFIG_DECIMAL_INVALID: %s value=%r; defaulting to %s", path, val, default_on_error)
                return default_on_error
            self.logger.error("CONFIG_DECIMAL_INVALID: %s value=%r", path, val)
            raise ConfigContractError(
                path=path, why=f"Invalid decimal value: {val!r}") from e
    ...
    def on_portfolio(self, portfolio_state: Dict[str, Any]) -> None:
        ...
        open_positions_margin_usd = portfolio_state[
            "open_positions_margin_usd"] if "open_positions_margin_usd" in portfolio_state else "0"
    ...
    def can_open(
        ...
        is_flip: bool = False, flip_fraction: Decimal = Decimal("1"),
    ) -> Dict[str, Any]:
        """
        Check if opening a position is allowed based on exposure limits.

        DEF-E06: flip_fraction controls how much of the existing position margin is
        freed by the opposite-side order. A tiny opposite order (e.g., 10 contracts
        against a 100-contract position) should only free 10/100 = 10% of margin.
        Previously is_flip=True always freed 100% regardless of order size.

        Args:
            flip_fraction: Decimal in [0, 1]. 1.0 = full position flip or close;
                           < 1.0 = partial close proportional to order size.
                           Ignored when is_flip=False.
        ...
        if last_ts_ms == 0 or stale_sec > stale_ttl:
            # Time-domain telemetry only: stale_sec is not used in financial math.
        ...
        open_positions_margin_usd = _d(
            portfolio_state.get("open_positions_margin_usd"))
        ...
        for p in positions:
            ...
            current_symbol_margin = sym_notional / \
                ref_leverage if ref_leverage else Decimal("0")
        ...
        if open_positions_usd <= 0 and positions:
            try:
                open_positions_usd = sum(
                    abs(_d(p.get("net_position"))) *
                    _d(p.get("avg_entry_price"))
                    for p in positions
        ...
        post_long_m = sum(
            ...
            # exp_ts is a time-domain timestamp; float conversion here is not part of
            # any financial arithmetic and is kept only for wall-clock comparison.
        ...
        total_pending_margin = pending_long_m + \
            pending_short_m + post_long_m + post_short_m
        ...
        if soft_mode in {"clip", "reject"}:
            ...
            if clip_res.clipped_notional is not None and clip_res.clipped_notional < order_notional_abs:
                ...
                clip_payload = {
                    ...
                    "clip_reasons": list(clip_res.clip_reasons),
                    # Helpful debug
                    "original_notional": str(requested_notional_abs)
                }

        # --- 4. Hard Limit Enforcement (domains.execution_position.exposure_guard) ---
        order_margin = order_notional_abs / \
            ref_leverage if ref_leverage else Decimal("0")

        # A) Max Portfolio Fraction (NOTIONAL-based, projected)
        projected_notional = open_positions_usd + \
            pending_notional + post_notional + order_notional_abs
        if is_flip:
            projected_notional -= current_symbol_notional * \
                flip_fraction  # DEF-E06: size-aware
        ...
        if projected_notional > p_frac_limit:
            return {
                ...
                "projected_notional": _decimal_payload(projected_notional),
                "limit": _decimal_payload(p_frac_limit),
            }

        # B) Max Equity Utilization (MARGIN-based, projected)
        total_margin_used = open_positions_margin_usd + \
            total_pending_margin + order_margin
        if is_flip:
            total_margin_used -= current_symbol_margin * \
                flip_fraction  # DEF-E06: size-aware
        ...
        if total_margin_used > equity_margin_limit:
            return {
                ...
                "actual": _decimal_payload(total_margin_used),
                "limit": _decimal_payload(equity_margin_limit),
            }

        # C) Long/Short Utilization (MARGIN-based)
        new_long_m = long_margin + pending_long_m + post_long_m + \
            (order_margin if order_side == "BUY" else Decimal("0"))
        new_short_m = short_margin + pending_short_m + post_short_m + \
            (order_margin if order_side == "SELL" else Decimal("0"))

        if is_flip:
            if current_symbol_side == "BUY":
                new_long_m -= current_symbol_margin * flip_fraction  # DEF-E06: size-aware
            elif current_symbol_side == "SELL":
                new_short_m -= current_symbol_margin * flip_fraction  # DEF-E06: size-aware
        ...
        if new_long_m > long_limit:
            return {"allowed": False, "reason": "LONG_UTILIZATION_BREACH", "actual": _decimal_payload(new_long_m), "limit": _decimal_payload(long_limit)}
        if new_short_m > short_limit:
            return {"allowed": False, "reason": "SHORT_UTILIZATION_BREACH", "actual": _decimal_payload(new_short_m), "limit": _decimal_payload(short_limit)}
        ...
        if min_m > Decimal("0"):
            ...
            if ratio > self.max_directional_ratio:
                return {"allowed": False, "reason": "DIRECTIONAL_RATIO_BREACH", "ratio": _decimal_payload(ratio), "max": _decimal_payload(self.max_directional_ratio)}
        ...
        projected_symbol_margin = symbol_margin_est + \
            pending_symbol_m + post_symbol_m + order_margin
        ...
        if projected_symbol_margin > concentration_limit:
            return {
                ...
                "projected_symbol_margin": _decimal_payload(projected_symbol_margin),
                "limit": _decimal_payload(concentration_limit),
            }
    ...
    def reserve(
        ...
        if side not in allowed_sides:
            raise ValueError(
                f"Invalid order side: '{side}'. Expected one of {allowed_sides}")
        ...
        if key in self.state.pending_exposure or key in self.state.reservations:
            self.logger.debug(
                f"EXPOSURE_RESERVE_IDEMPOTENT: key={key} already reserved")
            return
        ...
        order_logger.write({
            ...
            "symbol": symbol,
            "side": side,
            # Log margin amount without float conversion
            "quantity": _decimal_payload(reserve_margin),
            "source_fsm": "ExposureGuard",
            "reservation_id": key,
            "metadata": {
                "reservation_created": True,
                "reduce_only": reduce_only,
                "reserve_margin": _decimal_payload(reserve_margin),
                "leverage": _decimal_payload(symbol_leverage),
                "side": side
            }
        })
    ...
    def on_fill(self, key: str, notional_usd: Decimal, symbol: str = "", side: str = "SELL") -> None:
        ...
        """
        if not symbol:
            symbol = str(self.state.pending_exposure.get(
                key, {}).get("symbol", "") or "").strip()
        side = str(side or self.state.pending_exposure.get(
            key, {}).get("side", "SELL")).upper()
        allowed_sides = {"BUY", "SELL", "LONG", "SHORT"}
        if side not in allowed_sides:
            raise ValueError(
                f"Invalid order side: '{side}'. Expected one of {allowed_sides}")
    ...
    def record_postfill_hold(
        ...
        *,
        ...
        filled_margin = normalized_notional / \
            symbol_leverage if symbol_leverage else Decimal("0")
    ...
    def get_exposure_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "reservations_usd": _decimal_payload(total_pending),
            "reservations_margin_usd": _decimal_payload(total_pending_margin),
            "postfill_hold_count": total_postfill,
            "postfill_hold_margin_usd": _decimal_payload(total_postfill_margin),
    ...
    def on_regime_changed(self, bucket: Union[ExecutionRegimeBucket, str]) -> None:
        ...
        if isinstance(bucket, str):
            try:
                ...
                self.logger.warning(
                    f"EP-01: Unknown regime bucket string '{bucket}', treating as UNCERTAIN")
        ...
        self.logger.info(
            f"EP-01 REGIME_ADAPTED: bucket={bucket.value} → directional_ratio_max={_decimal_2dp(new_ratio)} "
            f"(base={_decimal_2dp(base_ratio)}, delta={_decimal_2dp(delta)})"
        )
...
def __getattr__(name: str) -> Any:
    if name == "DomainConfigResolver":
        return _DomainConfigResolver
