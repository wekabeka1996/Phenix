...
Notes:
- "client:" and "entry:" key writes are intentional no-ops: these values are derived from
  order rows on read, so storing them separately would create a split-brain risk.
- "order:" deletes perform a soft-delete (status=CANCELLED) to prevent stale ACTIVE reads.
...
class LedgerStoreAdapter:
    ...
    def put(self, key: str, value: Any) -> None:
        # Only persist meaningful keys; "client:" and "entry:" are derived from order rows on read.
        if key.startswith("order:"):
            order_id = key.split(":", 1)[1]
