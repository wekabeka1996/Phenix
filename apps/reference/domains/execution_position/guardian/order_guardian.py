...
# Keep the resolved repo root stable after the Phase 8C guardian package move.
...
class OrderGuardian:
    ...
    def _iter_symbols_for_poll(self) -> List[str]:
        ...
        try:
            # DEF-E13: Do NOT access store._data directly — it is a private implementation
            # detail of InMemoryStore/LedgerStoreAdapter. Use the public interface instead.
            # If the store exposes get_all_tracked_symbols(), prefer that.
            if hasattr(self.store, "get_all_tracked_symbols"):
                for sym in (self.store.get_all_tracked_symbols() or []):
                    if sym:
                        symbols.add(str(sym).upper())
            elif hasattr(self.store, "get_all"):
                for meta in (self.store.get_all() or {}).values():
                    if isinstance(meta, dict):
                        sym = meta.get("symbol")
                        if sym:
                            symbols.add(str(sym).upper())
            # DEF-E13: If neither public method exists, skip rather than access _data.
            # This is a contract gap — add get_all_tracked_symbols() to StoreProtocol.
        except Exception:
            pass
