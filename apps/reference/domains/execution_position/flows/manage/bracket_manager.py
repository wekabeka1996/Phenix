...
LOG = logging.getLogger(
    "apps.reference.domains.execution_position.bracket_manager"
)
...
class BracketManager:
    ...
    async def place_deferred_brackets(
        ...
        cleanup_other = getattr(
            self._fsm.order_guardian,
            "cleanup_other_brackets_for_symbol",
            None,
        )
        if callable(cleanup_other):
            try:
                cancelled = int(
                    await cleanup_other(
                        symbol=symbol,
                        keep_parent_order_id=str(entry_order_id),
                    )
                )
            except Exception as exc:
                LOG.debug(
                    "[LIMIT-DEFERRED] pre-placement cleanup_other_brackets_for_symbol failed for %s: %s",
                    symbol,
                    exc,
                )
            else:
                if cancelled > 0:
                    LOG.info(
                        "[LIMIT-DEFERRED] Pre-placement cleanup cancelled %s stale brackets for %s",
                        cancelled,
                        symbol,
                    )
        ...
        def _response_summary(response: Any) -> Optional[Dict[str, Any]]:
            ...
            return {key: _stringify(response.get(key)) for key in keys}
        ...
        def _build_attempt_payload(
            *,
            ...
            return {
                ...
                "symbol": str(symbol),
                "side": str(child_side),
                "stop_price": _stringify(stop_price),
                "client_order_id": str(client_order_id),
                "close_position": True,
                "reduce_only": True,
            }

        child_outcomes: Dict[str, Dict[str, Any]] = {
            "SL": {
                "kind": "SL",
                "submission_attempted": False,
                "attempt_count": 0,
                "attempt_payloads": [],
                "client_order_id": str(sl_id),
                "adapter_outcome_class": "not_attempted",
            ...
            "TP2": {
                ...
                "adapter_outcome_class": "not_configured",
                "response_present": False,
                "response_summary": None,
                "order_id": None,
                "exchange_status": None,
        ...
        sl_resp = None
        tp_resp = None
        sl_error_code: Optional[int] = None
        tp_error_code: Optional[int] = None
        sl_error_msg: Optional[str] = None
        tp_error_msg: Optional[str] = None
        try:
            child_outcomes["SL"]["submission_attempted"] = True
            child_outcomes["SL"]["attempt_count"] = int(
                child_outcomes["SL"]["attempt_count"]
            ) + 1
            child_outcomes["SL"]["attempt_payloads"].append(
            ...
            self._fsm._orphan_metrics["tp_sl_placed_success"] += 1
            sl_order_id = str(sl_resp["orderId"])
            child_outcomes["SL"]["adapter_outcome_class"] = "accepted"
            child_outcomes["SL"]["response_present"] = True
            child_outcomes["SL"]["response_summary"] = _response_summary(
                sl_resp)
            child_outcomes["SL"]["order_id"] = sl_order_id
            child_outcomes["SL"]["exchange_status"] = _stringify(
                sl_resp.get("status")
            )
            child_outcomes["SL"]["exchange_code"] = _stringify(
                sl_resp.get("code")
            )
            child_outcomes["SL"]["exchange_message"] = _stringify(
                sl_resp.get("msg")
            )
            child_outcomes["SL"]["missing_order"] = False
            self._fsm.correlation_store.put_sl_tp_ack(
            ...
            sl_error_code = int(getattr(e, "code", 0) or 0) or None
            sl_error_msg = str(getattr(e, "msg", "") or str(e))
            child_outcomes["SL"]["adapter_outcome_class"] = "exception"
            child_outcomes["SL"]["response_present"] = False
            child_outcomes["SL"]["response_summary"] = None
            child_outcomes["SL"]["order_id"] = None
            child_outcomes["SL"]["exchange_status"] = None
            child_outcomes["SL"]["exchange_code"] = sl_error_code
            child_outcomes["SL"]["exchange_message"] = sl_error_msg
            child_outcomes["SL"]["error_class"] = e.__class__.__name__
            child_outcomes["SL"]["missing_order"] = True
            LOG.error(
                f"❌ [LIMIT-DEFERRED] Failed to place SL for {symbol}: {e}")

        try:
            child_outcomes["TP1"]["submission_attempted"] = True
            child_outcomes["TP1"]["attempt_count"] = int(
                child_outcomes["TP1"]["attempt_count"]
            ) + 1
            child_outcomes["TP1"]["attempt_payloads"].append(
                _build_attempt_payload(
                    kind="TP1",
                    stop_price=tp,
                    child_side=tp_side,
            ...
            self._fsm._orphan_metrics["tp_sl_placed_success"] += 1
            tp_order_id = str(tp_resp["orderId"])
            child_outcomes["TP1"]["adapter_outcome_class"] = "accepted"
            child_outcomes["TP1"]["response_present"] = True
            child_outcomes["TP1"]["response_summary"] = _response_summary(
                tp_resp)
            child_outcomes["TP1"]["order_id"] = tp_order_id
            child_outcomes["TP1"]["exchange_status"] = _stringify(
                tp_resp.get("status")
            )
            child_outcomes["TP1"]["exchange_code"] = _stringify(
                tp_resp.get("code")
            )
            child_outcomes["TP1"]["exchange_message"] = _stringify(
                tp_resp.get("msg")
            )
            child_outcomes["TP1"]["missing_order"] = False
            self._fsm.correlation_store.put_sl_tp_ack(
            ...
            if e.code == -2021:
                ...
                try:
                    ...
                    tp_error_code = int(getattr(e2, "code", 0) or 0) or None
                    tp_error_msg = str(getattr(e2, "msg", "") or str(e2))
                    child_outcomes["TP1"]["adapter_outcome_class"] = "exception"
                    child_outcomes["TP1"]["response_present"] = False
                    child_outcomes["TP1"]["response_summary"] = None
                    child_outcomes["TP1"]["order_id"] = None
                    child_outcomes["TP1"]["exchange_status"] = None
                    child_outcomes["TP1"]["exchange_code"] = tp_error_code
                    child_outcomes["TP1"]["exchange_message"] = tp_error_msg
                    child_outcomes["TP1"]["error_class"] = e2.__class__.__name__
                    child_outcomes["TP1"]["missing_order"] = True
                    LOG.error(
                        f"❌ [LIMIT-DEFERRED] TP retry failed for {symbol}: {e2}")
            else:
                tp_error_code = int(getattr(e, "code", 0) or 0) or None
                tp_error_msg = str(getattr(e, "msg", "") or str(e))
                child_outcomes["TP1"]["adapter_outcome_class"] = "exception"
                child_outcomes["TP1"]["response_present"] = False
                child_outcomes["TP1"]["response_summary"] = None
                child_outcomes["TP1"]["order_id"] = None
                ...
                child_outcomes["TP1"]["exchange_code"] = tp_error_code
                child_outcomes["TP1"]["exchange_message"] = tp_error_msg
                child_outcomes["TP1"]["error_class"] = e.__class__.__name__
                child_outcomes["TP1"]["missing_order"] = True
                LOG.error(
        ...
        if placement_success:
            ...
            common_details = {
                ...
                "sl_error_code": sl_error_code,
                "tp_error_code": tp_error_code,
                "sl_error_msg": sl_error_msg,
                "tp_error_msg": tp_error_msg,
                "correlation": {
                    "rid": _stringify(rid),
                    "corr_id": _stringify(corr_id),
                    "entry_order_id": _stringify(entry_order_id),
                    "entry_client_order_id": _stringify(entry_client_order_id),
                },
                "child_outcomes": child_outcomes,
            }

            has_duplicate_closeposition_reject = any(
                code == -4130 for code in (sl_error_code, tp_error_code)
            )

            if not sl_present and not tp_present:
                # Both legs failed — position is fully unprotected.
                if has_duplicate_closeposition_reject:
                    why_code = "BRACKET_DEFERRED_EXISTING_CLOSEPOSITION_ORDER"
                    reason = "deferred_both_failed_existing_closeposition_order_conflict"
                    failure_class = "exchange_reject_duplicate_closeposition_direction"
                else:
