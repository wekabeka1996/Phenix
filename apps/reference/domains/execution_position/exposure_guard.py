...
from apps.reference.config_risk import resolve_risk_soft_limits
...
def _normalize_pct_decimal(value: Any, default: str = "0.0") -> Decimal:
    """Convert percent-style config values to ratio decimals (0-1 == 0-100%)."""
...
class ExposureGuard:
    ...
    def __init__(self, config: Dict[str, Any], fsm: Optional[Any] = None):
        ...
        # Percent fields now defined on 0-100 scale; normalize to ratios for calculations
        self._pct_normalizer = lambda value, default: _normalize_pct_decimal(
            value, default)

        self._exposure_policy = resolve_exposure_policy(config)

        # Log exposure policy source and key values
        self.logger.info(
            "EXPOSURE_POLICY_RESOLVED",
            extra={
                "source": self._exposure_policy.source,
                "max_equity_utilization_ratio": float(self._exposure_policy.caps.max_equity_utilization_ratio),
                "max_portfolio_fraction": float(self._exposure_policy.caps.max_portfolio_fraction),
                "max_side_utilization_long": float(self._exposure_policy.caps.max_side_utilization_ratio["long"]),
                "max_side_utilization_short": float(self._exposure_policy.caps.max_side_utilization_ratio["short"]),
                "max_directional_ratio": float(self._exposure_policy.caps.max_directional_ratio),
                "per_symbol_cap_ratio": float(self._exposure_policy.caps.per_symbol_cap_ratio),
            }
        )

        caps = self._exposure_policy.caps
        self.max_equity_utilization_pct = caps.max_equity_utilization_ratio
        self.max_portfolio_fraction = caps.max_portfolio_fraction
        self.max_long_utilization_pct = caps.max_side_utilization_ratio["long"]
        self.max_short_utilization_pct = caps.max_side_utilization_ratio["short"]
        self.max_directional_ratio = caps.max_directional_ratio
        self.per_symbol_cap_pct = caps.per_symbol_cap_ratio

        reservations = self._exposure_policy.reservations
        self.pending_ttl_sec = reservations.pending_ttl_sec
        self.post_fill_hold_ttl_sec = reservations.post_fill_hold_ttl_sec
        self.positions_stale_ttl_sec = reservations.positions_stale_ttl_sec

        self.count_pending_orders = self._exposure_policy.count_pending_orders
        self.exclude_reduce_only = self._exposure_policy.exclude_reduce_only
        ...
        self.logger.info(
            f"🔧 CONFIG: directional_ratio_max={self.max_directional_ratio}, "
            f"equity_utilization_pct={self.max_equity_utilization_pct}, "
            f"side_long_pct={self.max_long_utilization_pct}, "
            f"side_short_pct={self.max_short_utilization_pct}"
        )
    ...
    def _load_soft_limit_config(self) -> SoftLimitConfig:
        ...
        Resolved via config_risk resolver (config v2 primary).
        """
        try:
            soft_limits = resolve_risk_soft_limits(self.config)
        except Exception as exc:
            self.logger.error(
                "SOFT_LIMIT_CONFIG resolver failed (%s); falling back to defaults", exc
            )
            soft_limits = None
        ...
        if soft_limits is None:
            mode = "clip"
            clip_min = Decimal("10")
            dir_ratio = Decimal("3.0")
            side_exp = Decimal("600")
            margin_exp = Decimal("1100")
        else:
            mode = soft_limits.mode
            clip_min = Decimal(str(soft_limits.clip_min_notional_usdt))
            dir_ratio = Decimal(str(soft_limits.directional_ratio_max))
            side_exp = Decimal(str(soft_limits.side_exposure_usdt))
            margin_exp = Decimal(str(soft_limits.margin_exposure_usdt))
    ...
    def _load_fallback_config(self) -> Dict[str, Any]:
        """Expose fallback configuration resolved via central policy."""
        fallback = self._exposure_policy.fallback
        backoff_ms = list(fallback.backoff_sequence())
        config = {
            "policy": fallback.policy,
            "risk_reduction_pct": fallback.risk_reduction_pct,
            "backoff_ms": backoff_ms,
            "enabled": fallback.enabled,
            "max_attempts": fallback.max_attempts,
        }
        self.logger.info(
            "FALLBACK_CONFIG loaded: policy=%s, risk_reduction_pct=%s, backoff_ms=%s, "
            "enabled=%s, max_attempts=%s",
            fallback.policy,
            fallback.risk_reduction_pct,
            backoff_ms,
            fallback.enabled,
    ...
    def can_open(
        ...
        with self._state_lock:
            ...
            try:
                equity_free_usdt_str = portfolio_state.get(
                    "equity_free_usdt", "0")
                open_positions_margin_usd_str = portfolio_state.get(
                    "open_positions_margin_usd", None)
                # Fallback: if 'open_positions_margin_usd' not provided, infer from
                # 'open_positions_usd' dividing by default leverage for the symbol.
                if open_positions_margin_usd_str is None:
                    open_positions_usd_str = portfolio_state.get(
                        "open_positions_usd") or "0"
                    try:
                        default_leverage = self.resolve_symbol_leverage(symbol)
                    except Exception:
                        default_leverage = Decimal("20")
                    # Avoid division by zero
                    if Decimal(str(default_leverage)) and Decimal(str(default_leverage)) > 0:
                        open_positions_margin_usd = (
                            Decimal(str(open_positions_usd_str)) /
                            Decimal(str(default_leverage))
                        )
                    else:
                        open_positions_margin_usd = Decimal("0")
                else:
                    open_positions_margin_usd = (
                ...
                # `open_positions_margin_usd` is now set above in either branch
    ...
    def on_fill(self, key: str, notional_usd: Decimal, symbol: str = "", side: str = "SELL") -> None:
        ...
        with self._state_lock:
            self.logger.debug(
                f"on_fill called with key={key}, reservations_keys={list(self.state.reservations.keys())}")
            if key in self.state.reservations:
                # Calculate filled margin. Prefer the originally reserved margin
                # from pending_exposure if present to avoid changing the exposure
                # footprint after a reservation -> fill transition.
                existing_pending = self.state.pending_exposure.get(key)
                if existing_pending and "margin" in existing_pending:
                    filled_margin = existing_pending.get("margin")
                    # Preserve leverage if available from pending_exposure entry
                    symbol_leverage = existing_pending.get(
                        "leverage", Decimal("1"))
                else:
                    symbol_leverage = self.resolve_symbol_leverage(
                        symbol) if symbol else Decimal("1")
                ...
                self.state.pending_exposure.pop(key, None)
                self.logger.debug(
                    f"on_fill moved key={key} to postfill; now reservations_keys={list(self.state.reservations.keys())}")
