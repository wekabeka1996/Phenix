...
class MRStrategyConfig:
    ...
    # Confidence formula parameters — move to config to allow tuning without code changes
    ...
    def _evaluate_signal(
        ...
        if pct_b < entry_threshold:
            ...
            confidence = self.config.confidence_base + \
                (entry_threshold - pct_b) * self.config.confidence_distance_mult
        ...
        # SHORT: price above upper band
            ...
            confidence = self.config.confidence_base + \
                ...
                self.config.confidence_distance_mult
    ...
    def force_close_all(self, timestamp_ms: int) -> Dict[str, Optional[Bar]]:
        ...
        for symbol, state in self._states.items():
            bar = state.resampler.force_close(
                timestamp_ms) if state.resampler else None
            if bar:
                state.add_bar(bar)
            result[symbol] = bar
        return result
