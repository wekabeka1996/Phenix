...
class WebSocketAggregator:
    ...
    def on_book_ticker(self, symbol: str, bid_price: str, bid_size: str,
                       ask_price: str, ask_size: str, ts: int) -> None:
        """
    ...
    def on_trade(self, symbol: str, price: str, quantity: str,
                 is_buyer_maker: bool, ts: int) -> None:
        """
    ...
    def get_market_tick(self, symbol: str) -> Optional[Dict[str, Any]]:
        ...
        tfi = (
            ...
            if total_trades > 0 else decimal.Decimal(0)
        )

        # delta_price
        if len(state['prices']) >= 2:
            prev_price = state['prices'][-2]
            delta_price = (state['latest_price'] - prev_price) / prev_price if prev_price > 0 else decimal.Decimal(0)
        else:
