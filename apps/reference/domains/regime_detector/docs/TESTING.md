...
- `tests/test_regime_detector_integration.py` - інтеграційні тести в складі FSM.
...
##### test_initialization
Перевірка коректного завантаження конфігурації `AuroraConfig` та ініціалізації буферів.

##### test_sma_trend_detection
Перевірка визначення `TREND_UP` та `TREND_DOWN` на основі SMA crossover.

##### test_volatility_regime_detection
Перевірка перемикання в `HIGH_VOLATILITY` при різкому зростанні ATR відносно baseline.

##### test_mean_reversion_detection
Перевірка визначення `MEAN_REVERSION` при низькому спреді SMA та ціні поблизу середніх.

##### test_data_quality_gate (TASK24)
Перевірка скидання режиму в `UNCERTAIN` при отриманні застарілих даних (`stale_features`) або нульової ціни.

##### test_warmup_logic
Перевірка прапорця `full_ready` при поступовому заповненні буферів SMA та ATR.

## Як запускати тести

```bash
# Запуск всіх тестів домену
pytest tests/test_regime_detector.py

# Запуск з логуванням
pytest -s tests/test_regime_detector.py
```
