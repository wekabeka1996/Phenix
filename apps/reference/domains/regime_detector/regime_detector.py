...
class RegimeDetector:
    ...
    def handle_event(self, event: Message) -> None:
        ...
        atr_14_sma_100 = (
            Decimal(features.get("atr_14_sma_100", "0"))
        ...
        if (
            ...
            threshold_multiplier = Decimal(
                str(volatility_config.get("threshold_multiplier", 2.0))
            )
            low_vol_multiplier = Decimal(
                str(volatility_config.get("low_vol_multiplier", 0.5))
            )
            ...
            if volatility_ratio > threshold_multiplier:
                ...
                confidence = min(
                    Decimal("0.95"), Decimal("0.5") + excess_volatility * Decimal("2.0")
                )
            ...
            # LOW_VOLATILITY: ATR significantly below its long-term average
                ...
                confidence = min(
                    Decimal("0.95"), Decimal("0.5") + calm_factor * Decimal("3.0")
                )
        ...
        if regime == "UNCERTAIN":  # Only check if volatility didn't trigger
            sma_spread = (
                abs(sma_short - sma_long) / sma_long if sma_long > 0 else Decimal("1.0")
            )
            price_deviation_short = (
                abs(price - sma_short) / sma_short if sma_short > 0 else Decimal("1.0")
            )
            price_deviation_long = (
                abs(price - sma_long) / sma_long if sma_long > 0 else Decimal("1.0")
            )
            ...
            if (
                ...
                confidence = min(
                    Decimal("0.95"), Decimal("0.5") + tightness * Decimal("100.0")
                )
