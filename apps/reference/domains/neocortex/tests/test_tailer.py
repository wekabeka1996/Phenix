...
def sample_events():
    ...
    return [
        {"verb": "FEATURES_CALCULATED", "timestamp": 1000, "payload": {"timestamp": 1000, "features": {"a": "1"}}},
        {"verb": "FEATURES_CALCULATED", "timestamp": 1001, "payload": {"timestamp": 1001, "features": {"a": "2"}}},
        {"verb": "TRADE_INTENT", "timestamp": 1002, "payload": {"symbol": "BTC"}},  # Should be filtered
        {"verb": "FEATURES_CALCULATED", "timestamp": 1003, "payload": {"timestamp": 1003, "features": {"a": "3"}}},
    ]
...
def test_tailer_appends_new_data(temp_wal_dir, temp_state_file):
    ...
    async def run_test():
        ...
        with open(wal_file, 'a') as f:
            f.write(json.dumps({"verb": "FEATURES_CALCULATED", "timestamp": 2000, "payload": {"timestamp": 2000}}) + '\n')
...
def test_tailer_file_rotation(temp_wal_dir, temp_state_file):
    ...
    async def run_test():
        ...
        write_wal_file(wal_file2, [
            {"verb": "FEATURES_CALCULATED", "timestamp": 2000, "payload": {"timestamp": 2000}}
        ])
...
def test_tailer_state_persistence(temp_wal_dir, temp_state_file, sample_events):
    ...
    assert len(handled_events) == 0, f"Expected 0 on restart, got {len(handled_events)}"
...
def test_tailer_handles_incomplete_json(temp_wal_dir, temp_state_file):
    ...
    with open(wal_file, 'w') as f:
        f.write('{"verb":"FEATURES_CALCULATED","timestamp":1000,"payload":{"timestamp":1000}}\n')
    ...
    async def run_test():
        ...
        with open(wal_file, 'a') as f:
            f.write('CULATED","timestamp":2000,"payload":{"timestamp":2000}}\n')
