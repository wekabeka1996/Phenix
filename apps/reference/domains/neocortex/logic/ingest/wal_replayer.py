...
class WALReplayer:
                ...
                async with aiofiles.open(file_path, 'r', encoding='utf-8', errors='replace') as f:
                    async for line in f:
                        ...
                        if line_count % 10000 == 0:
                            logger.info(
                                f"  {file_path.name}: {line_count} lines processed")
            else:
    ...
    async def _process_line(self, line: str):
        ...
        try:
            ...
            # Filter by verb
            verb = event.get("verb") or event.get(
                "event_type") or event.get("type")
            ...
            self._events_malformed += 1
            if self._events_malformed <= 3 or self._events_malformed % 1000 == 0:
                logger.warning(
                    "Malformed WAL JSON line skipped: count=%d error=%s",
                    self._events_malformed,
                    exc,
                )
        except Exception as e:
    ...
    def stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "events_malformed": self._events_malformed,
