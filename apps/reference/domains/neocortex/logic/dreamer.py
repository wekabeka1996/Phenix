...
class Dreamer:
    ...
    def _process_episode(
        ...
        if encoder is not None:
            ...
            raise ValueError(
                "Dreamer requires an encoder; raw features are not latent state")

        # Get action (side mapping)
        side = getattr(episode, 'side', None)
        if not isinstance(side, str) or side.upper() not in self.ACTION_MAP:
            raise ValueError(f"Dreamer episode has invalid side: {side!r}")
        action = self.ACTION_MAP[side.upper()]

        # Get reward
        raw_reward = getattr(episode, 'reward', None)
        if raw_reward is None or isinstance(raw_reward, bool):
            raise ValueError("Dreamer episode reward is required")
        reward = float(raw_reward)
        if not np.isfinite(reward):
            raise ValueError(
                f"Dreamer episode reward must be finite: {raw_reward!r}")
    ...
    def consolidate_sequences(
        ...
        for i, z_seq in enumerate(z_sequences):
            ...
            values = value_sequences[i] if value_sequences else [
                0.0] * len(rewards)
    ...
    def _run_pruning(self):
        ...
        edges_pruned = self.graph.prune_low_count_edges(
            self.min_edge_count_for_prune)

        # Remove orphan nodes
        nodes_pruned = self.graph.prune_orphan_nodes()

        logger.info(
            f"Pruning complete: {edges_pruned} edges, {nodes_pruned} nodes removed")
