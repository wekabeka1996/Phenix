...
class MeanReversionHandler:
    ...
    def _init_strategies(self) -> None:
        ...
        for symbol in self._enabled_symbols:
            ...
            config.confidence_base = Decimal(
                str(base_strat_cfg.confidence_base))
            config.confidence_distance_mult = Decimal(
                str(base_strat_cfg.confidence_distance_mult))
            config.rsi_confidence_boost = Decimal(
                str(base_strat_cfg.rsi_confidence_boost))
    ...
    def _on_regime_detected(self, event: Message) -> None:
        try:
            ...
            if isinstance(pld, dict):
                ...
                regime = str(pld.get("regime") or pld.get(
                    "overall_regime") or "")
            else:
                symbol = str(getattr(pld, "symbol", "") or "")
                regime = str(aget(pld, "regime", "") or aget(
                    pld, "overall_regime", "") or "")
    ...
    def _on_process_strategy(self, event: Message) -> None:
        ...
        try:
            ...
            bar_close_ts = pld.get("bar_close_ts") if isinstance(
                pld, dict) else getattr(pld, "bar_close_ts", None)
            if not bar_close_ts:
                self._stats["bars_rejected_missing_tf"] += 1
                self.logger.warning(
