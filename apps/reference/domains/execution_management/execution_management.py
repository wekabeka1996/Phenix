...
class ExecutionManagement:
    ...
    def on_trade_intent(self, event: Message) -> None:
        ...
        self.logger.info(f"Forwarding trade intent for {symbol} to execution_position FSM")

        chain_logger.info("Event forwarded", extra={
            "rid": rid,
            "event_type": "EVT:TRADE_INTENT_PROPOSED",
            "domain": "execution_management",
            "symbol": symbol,
        ...
        self.logger.info(f"Trade intent processed: {trade_intent.get('side')} {trade_intent.get('order', {}).get('qty')} {symbol}")