...
class WinSafeRotatingFileHandler(RotatingFileHandler):
    ...
    thread still holds the file handle open during rollover.  We catch it in
    rotate() so logging continues uninterrupted and the next tick will retry.
    """
    ...
    def rotate(self, source: str, dest: str) -> None:
        try:
            ...
            pass  # File locked by another thread — skip rotation this cycle
...
def setup_logging(
    ...
    *,
    ...
    if obs_logging.core.enabled:
        ...
        core_handler = WinSafeRotatingFileHandler(
            core_path,
            maxBytes=max_bytes,
    ...
    domain_name_mapping = {
        ...
        # Special case: uses custom logger name
        "mean_reversion": "domain_mean_reversion",
    }
    ...
    for domain_name, domain_cfg in obs_logging.domains.items():
        ...
        # Create handler
        domain_handler = WinSafeRotatingFileHandler(
            domain_log_path,
        ...
        logger_prefix = domain_name_mapping.get(
            domain_name, f"apps.reference.domains.{domain_name}")
    ...
    if obs_logging.event_chain.enabled:
        ...
        event_handler = WinSafeRotatingFileHandler(
            event_chain_path,
        ...
        event_handler.addFilter(
            lambda record: hasattr(
                record, "rid") or record.name == "event_chain"
        )
