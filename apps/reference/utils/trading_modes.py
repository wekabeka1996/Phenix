...
def _get_v2_modes_cfg(config: Any) -> Optional[Dict[str, Any]]:
    """
    Returns config_v2.modes if it exists and is non-empty.
    Returns None if v2-config for modes is missing.
    """
...
def build_legacy_mode_mapping_from_v2(modes_cfg: Mapping[str, Any], profile_name: str) -> Dict[str, Any]:
    """
    Builds a structure equivalent to legacy trading.domain_configuration + trading.mode,
    based on config v2 (modes.yaml).
