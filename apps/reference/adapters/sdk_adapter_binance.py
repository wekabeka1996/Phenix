...
class SdkAdapterBinance(ExecutionAdapter):
    ...
    def __init__(self, mode: ExecutionMode):
        ...
        if mode == ExecutionMode.LIVE:
            raise InvalidModeError(
                operation="SdkAdapterBinance.__init__", mode="live", required_mode="paper"
            )
        ...
        try:
            ...
            self._client = Client(
                api_key=config.exchange_api_key, api_secret=config.exchange_api_secret, testnet=True
            )
    ...
    def _submit_impl(self, order: OrderDTO, client_order_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "remaining_qty": str(
                    Decimal(response.get("origQty", "0"))
                    - Decimal(response.get("executedQty", "0"))
                ),
            ...
            raise SDKError(operation="submit", sdk_code=code,
        ...
        try:
            ...
            try:
                if hasattr(config, 'trading') and config.trading:
                    ...
                    elif isinstance(config.trading, dict):
            ...
            # Use dynamic symbol from config instead of hardcoded BTCUSDT
            response = self._client.cancel_order(
                symbol=symbol, **params)  # type: ignore[union-attr]
            ...
            raise SDKError(operation="cancel",
                           sdk_code="CANCEL_FAILED", sdk_message=str(e)[:60])
    ...
    def stream(self) -> AsyncIterator[Dict[str, Any]]:
        ...
        async def _stream() -> AsyncIterator[Dict[str, Any]]:
            ...
            logger.warning(
                "SdkAdapterBinance.stream: WebSocket not implemented, returning empty stream"
            )
            if False:
                yield {}
