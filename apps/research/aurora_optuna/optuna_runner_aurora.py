...
def objective(trial, df_features, btc_df, bar_seconds=60):
    # EMA params
...
def run(symbol, year, month, n_trials=600, timeframe='1m'):
    ...
    study_name = f"aurora_{symbol}_{year}_{month}_{timeframe}"
    storage = f"sqlite:///aurora_{timeframe}.db"
    
    study = optuna.create_study(study_name=study_name, storage=storage, direction="maximize", load_if_exists=True)
    study.optimize(lambda t: objective(t, df, btc_df, bar_seconds), n_trials=n_trials)
    ...
    # We need to reconstruct the full params dict including allowed_regimes which are not in best_params directly (they are booleans)
    # Actually best_params HAS the booleans. We need to convert them to allowed_regimes list.
    
    # Helper to reconstruct params
    ...
    with open(f"best_aurora_{symbol}_{timeframe}.json", "w") as f:
...
if __name__ == "__main__":
    ...
    run(args.symbol, args.year, args.month, args.n_trials, args.timeframe)
