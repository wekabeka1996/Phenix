...
| Узел        | Файл  | Класс/Функции | Слушает | Эмитит | Ключ. поля |
| ----------- | ----- | ------------- | ------- | ------ | ---------- |
...
1. **Немає EVT:FEATURES_CALCULATED** для символа - DecisionMaking чекає на `on_features()` виклик
2. **Немає EVT:RISK_ASSESSMENT_COMPLETED** для символа - чекає на `on_risk()` виклик
3. **Немає PORTFOLIO_STATE_UPDATED** - чекає на `on_portfolio()` виклик
4. **TTL/устаревание** - немає явної логіки TTL, але дані можуть бути застарілими
5. **Неправильні підписки** - домени реєструються в main.py, але якщо FSM не ініціалізований правильно, події не доходять
6. **Конфігурація символів** - MarketDataConnector має `symbols_to_track: ["BTCUSDT", "ETHUSDT"]`, DecisionMaking працює з тими самими символами

## Список точних функцій/строк кода
...
- `apps/reference/domains/feature_engineering/feature_engineering.py:10` - `__init__()` - підписка на події
...
- `apps/reference/domains/risk_management/risk_management.py:25` - `__init__()` - підписка на події
...
- `apps/reference/domains/decision_making/decision_making.py:90` - `__init__()` - підписка на події

## Міні-прогон трассировки

Для перевірки потоку потрібно:

1. Запустити систему з live режимом
...
4. Якщо події доходять, але `features=False` - перевірити стан `symbol_states` в DecisionMaking

## Висновки

- **Карта потока точна** - всі вузли знайдені та проаналізовані
- **Причини `features=False`** - переважно відсутність подій від upstream доменів
- **Вузькі місця** - конфігурація символів узгоджена, але потрібна перевірка ініціалізації FSM та реєстрації слухачів
