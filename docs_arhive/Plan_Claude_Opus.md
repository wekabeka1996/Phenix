Сбор � ведений о рабочей обла� ти# План Міграції Aurora → vFoundation FSM Federation

## Головний План: `docs/PLAN_AURORA_TO_VFOUNDATION.md`
...
### `ops/LLM_LOG.jsonl` (приклад запи� ів)
...
| PH-INFRA | ⏸️ PENDING | - | - | - |
| PH-SKELETON | ⏸️ PENDING | - | - | - |
| PH-LAYERS | ⏸️ PENDING | - | - | - |
| PH-CONNECTORS | ⏸️ PENDING | - | - | - |
| PH-ORCHESTRATION | ⏸️ PENDING | - | - | - |
| PH-RESILIENCE | ⏸️ PENDING | - | - | - |
| PH-PRODUCTION | ⏸️ PENDING | - | - | - |
...
Цей комплек� ний план забезпечує:
1. **Машиночитабельні� ть** - в� і артефакти в JSON/YAML з � хемами
2. **Відтворювані� ть** - будь-яка LLM може продовжити з будь-якої точки
3. **Тра� овані� ть** - кожен крок логуєть� я в LLM_LOG.jsonl
4. **Які� ть** - жор� ткі гейти на кожній фазі
5. **Безпеку** - вбудовані перевірки на кожному етапі


## 🔍 Критичні прогалини в плані:

### 1. **Від� утня перевірка реальної інтеграції з Binance API** ❌
- План не включає те� тування **реального підключення** до WebSocket та REST API
- Немає кроку для перевірки **автентифікації** та отримання даних з те� тнету
- Не враховано можливі **rate limits** та обробку помилок API

### 2. **Не врахована міграція execution_engine** ❌  
- У `core/aurora/` є критичний домен `execution/` для виконання ордерів
- Без нього � и� тема може лише � по� терігати, але не торгувати
- Це блокує перехід від "read-only" до активної торгівлі

### 3. **Пропущена � и� тема WHY-chain tracing** ❌
- Згадуєть� я в документах, але не має конкретних кроків реалізації
- Критично для debugging та аудиту рішень FSM
- Потрібен окремий домен для збору та візуалізації WHY ланцюжків

### 4. **Від� утня � и� тема валідації торгових � игналів** ❌
- Немає перевірки � игналів перед виконанням
- Не реалізований механізм "dry run" для те� тування � тратегій
- Від� утня � имуляція торгів для безпечного те� тування

### 5. **Не враховано disaster recovery** ❌
- План не включає механізми відновлення пі� ля збоїв
- Немає checkpoint/restore для � тану FSM
- Від� утня обробка розривів з'єднання та втрати даних

### 6. **Пропущено метрики та моніторинг** ❌
- Немає Prometheus/Grafana інтеграції
- Від� утні performance метрики (latency, throughput)
- Не реалізований health check endpoint

### 7. **Не включено backtesting framework** ❌
- Важливо для валідації � тратегій на і� торичних даних
- Від� утня можливі� ть replay і� торичних подій
- Немає механізму порівняння live vs backtest результатів

## 📋 Оновлена дорожня карта з виправленнями:

### **Етап 0: API Validation & Connection Health** (НОВИЙ)
*Тривалі� ть: 2-3 дні*

- **T01**: Створити `ConnectionHealthCheck` домен для моніторингу яко� ті з'єднання
- **T02**: Реалізувати автоматичний reconnect з exponential backoff
- **T03**: Додати rate limit monitoring та throttling

### **Етап 1: Foundation Freeze** (оновлено)
*Додати до і� нуючого плану:*

- **T05**: Реалізувати WHY-chain collector для тра� ування рішень
- **T06**: Створити health check endpoints (`/health`, `/ready`)
- **T07**: Додати Prometheus метрики ек� портер

### **Етап 2: Total Awareness** (оновлено)
*Додати:*

- **T05**: Мігрувати `execution_engine` з core/aurora
- **T06**: Реалізувати dry-run режим для безпечного те� тування
- **T07**: Створити backtesting framework з replay механізмом

### **Етап 3.5: Resilience & Recovery** (НОВИЙ)
*Тривалі� ть: 1 тиждень*

- **T01**: Реалізувати checkpoint/restore для FSM � тану
- **T02**: Додати circuit breaker для запобігання ка� кадних збоїв  
- **T03**: Створити disaster recovery playbook з автоматизацією

### **Етап 4: Production Deployment** (НОВИЙ)
*Тривалі� ть: 1 тиждень*

- **T01**: Docker контейнеризація з multi-stage build
- **T02**: Kubernetes manifests для orchestration
- **T03**: CI/CD pipeline з автоматичними те� тами
- **T04**: Blue-green deployment � тратегія

## 🚨 Критичні залежно� ті, які потрібно вирішити ЗАРАЗ:

1. **Перевірити роботу Binance API ключів** - без цього нічого не працюватиме
2. **Визначити пріоритет**: observation-only чи full trading capability?
3. **Вибрати метод deployment**: local, cloud, чи hybrid?
