...
### Performance
- **Router p95**: Integrated у `/metrics`
- **Timeout rate**: Tracked
- **Queue depth**: Exposed

### Tracing
- **WHY-chain**: ≤80 chars, propagated через FSM
...
### High Priority
1. **mypy strict mode**: Зараз best-effort, потрібен strict
2. **CLI coverage**: 69% → 85% (schema/simulate commands)
3. **Platform tests**: Unix fcntl, потрібен Linux CI runner

### Medium Priority
4. **FSM edge cases**: Додаткові error path tests
5. **MetaFSM registry**: Schema versioning, canary support
6. **OrchestratorFSM**: RID lifecycle coordination

### Low Priority
7. **DR validation**: Full WAL replay test для RID lifecycle
8. **KMS integration**: Secrets management (зараз ENV)
...
3. **Canary deployment** — 10-20% traffic split для новых flows
