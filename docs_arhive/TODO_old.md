...
- **Problem**:                                          leverage            API,                                                                  
- **Risk Level**:      HIGH -                                                            (x50)                                   
- **Solution**:                                                                        leverage      margin_type            Binance API

### Changes

1. **Configuration** (`config/aurora/trading.yaml`):
   -                  `leverage: 50`        BTCUSDT      ETHUSDT
   -                  `margin_type: cross`                                         
...
4. **Documentation**:
   -     Research report: `docs/                  /LEVERAGE_RESEARCH_REPORT.md`
...
- 25x     3.6% distance        (risky, below 5%)
...
- **Problem**: BTCUSDT                                                     (qty=0),              ETHUSDT                 
- **Root Cause**: BTCUSDT                       `config/aurora/trading.yaml`     `instruments` section
- **Diagnostic Approach**:
  1.              DEBUG                       `decision_making.py` (position sizing + qty conversion)
  2.                         `aurora_trades.log` (455             ) -              ETHUSDT             
  3.                                             -                                         BTCUSDT
- **Solution**:              BTCUSDT      `trading.yaml`                          :
  - `min_qty: 0.001`
  - `lot_step: 0.001`
  - `tick_size: 0.01`
...
- **Modified Files**:
  ...
  - `config/aurora/trading.yaml` -              BTCUSDT instrument
  - `JOURNAL.md` -                                            RID: AURORA_QTY0_DIAG_V1
...
###         Task 01: DR Protocol Foundation (FSMP-RESILIENCE-T01)
...
- [x] **Part E**: Implement snapshot + WAL replay logic     DONE [2025-10-20]
  ...
  - **Function 1**: `find_latest_snapshot()` -                                         snapshot      mtime
  - **Function 2**: `replay_wal_after()` -                    WAL                       snapshot timestamp
...
- [x] **FSMP-P2-T03**: Feature Engineering Component     DONE; 3 tests pass; ruff ok; mypy ok; cov=82% (logic 100%) [2025-10-15]
  ...
  -        Coverage: 82% (FSMCore test class excluded, logic coverage 100%)
...
- [x] **FSMP-P3-T03**: Risk Management Component     DONE; 1 test pass; ruff ok; mypy ok; cov=78% (logic 100%) [2025-10-15]
  ...
  -     ruff: All checks passed
  -     mypy --strict: Success: no issues found
  -        Coverage: 78% (FSMCore test class excluded, logic coverage 100%)
  -      **DoD MET**: Component implemented, tests pass, quality gates met
...
- [x] **FSMP-P4-T03**: Position Tracking Component     DONE; 9 tests pass; ruff ok; mypy ok; cov=86% (logic 100%) [2025-01-15]
  ...
  -     ruff: All checks passed
  -     mypy --strict: Success: no issues found
  -        Coverage: 86% (FSMCore test class excluded, logic coverage 100%)
  -      **DoD MET**: Component implemented, tests pass, quality gates met
...
- [x] **FSMP-PERFECT-T07**: Position Tracking Logic Tests     DONE; 20 tests pass; position_tracking.py coverage 73%   90% (+17%) [2025-01-17]
  ...
  -        Uncovered: FSMCore helper class (lines 21, 25-27, 31-43), start() method (line 69), edge branch (line 228)
...
- [x] **FSMP-PERFECT-T09**: Decision Making Config Validation Tests     DONE; 8 tests pass; coverage 73%   75% (+2%) [2025-01-17]
  ...
  -        Remaining gaps: Lines 30, 114, 117-119, 155, 186-189, 212-214, 225-233, 236-237, 247-249, 307-308, 337-367
...
- [x] **FSMP-PERFECT-T10**: Decision Making Equity Validation Tests     DONE; 12 tests pass; coverage 75%   77% (+2%) [2025-01-17]
  ...
  -        Remaining gaps: Lines 30, 155, 186-189, 212-214, 225-233, 236-237, 247-249, 307-308, 337-367 (decision branches, exception handler, methods)
...
- [x] **FSMP-PERFECT-T11**: Decision Logic Branch Tests     DONE; 15/16 tests pass; coverage 77%   79% (+2%) [2025-01-17]
  ...
  -        Discovered bug: SHORT signals produce negative position size (Kelly calculation issue for sell side)     test marked as SKIPPED with explanation
  ...
  -        Remaining gaps: Lines 30, 155, 186-189, 225-233, 236-237, 247-249, 307-308, 337-367 (~41 lines, 21%)
...
- [x] **FSMP-PERFECT-T12**: SHORT Signal Bug Fix     DONE; 16/16 tests pass (100%); coverage 79% stable [2025-01-17]
  ...
  -        Remaining gaps: Lines 30, 188-191, 227-235, 238-239, 249-251, 309-310, 339-369 (~40 lines, 21%)
...
- [x] **FSMP-PERFECT-T13**: Helper Method Unit Tests     DONE; 38/38 tests pass (100%); coverage 79%   82% (+3%) [2025-01-17]
  ...
  -        Note: Expected ~16% gain (lines 339-369), but those are _validate_trade_intent method, not the extracted helpers; actual helper methods ~20 lines
  -        Remaining gaps: Lines 30, 228-236, 239-240, 250-252, 310-311, 340-370 (~37 lines, 8 percentage points to 90%)
...
- [x] **FSMP-PERFECT-T14**: Trade Intent Validation Tests     DONE; 70/70 tests pass (100%); coverage 82%   93% (+11%) **     TARGET EXCEEDED!** [2025-01-17]
  ...
  -        Remaining gaps: Lines 30, 228-236, 239-240, 250-252, 310-311 (15 lines, 7% to 100%)
...
- [x] **FSMP-EXECUTE-T03**: Decision to Execution Integration Test     DONE; test created and passed; bridge logic verified [2025-10-17]
  ...
  -      **DoD MET**: Integration test confirms "        "             ,                                                            
...
- [x] **FSMP-EXECUTE-T05**:                                                             Execution FSM     DONE;                                             ;                                              DEC:OPEN [2025-10-17]
  -                              ExecPosFSM.__init__                       config/fsm/shadow_mode                                 BinanceExecutionAdapter
  -                                              adapter.place_order()                               DEC:OPEN    handle_event
  -                      tests/integration/test_fsm_adapter_integration.py                                           
  -                            : FSM                                           CMD:OPEN     DEC:OPEN     adapter.place_order()
  -      **GATE PASSED**:                                                         ,                                                                                                     
...
- [x] **FSMP-P0-T03**:                                      89%     Merged in v2-clean
- [x] **FSMP-P0-T07**: Security & XAI Tightening (RBAC, signature, WHY-discipline)     Merged in v2-clean (tag: v2-clean-P0-PASS)

### P1 Task List
- [x] **FSMP-P1-T01**: ACL adapter        execution_position (exchange events     Message, shadow stub)
...
###      Task 01:                                                                              (FSMP-PROD-PREP-T01)

- [x] **Part A**:                                                                      (              ,             )     DONE [2025-01-25]
  - **Problem**:                                         `symbols = ["ethusdt"]`    `MarketDataConnector`                                                                             
  - **Solution**:                     `config/aurora/system.yaml`    fallback-              
  - **Changes**:
    -                    `config/aurora/system.yaml`                              `trading` (symbols_to_track, websocket_streams)
    -                        `MarketDataConnector.__init__`:                   `config['system']['trading']`, fallback-             (system.yaml     trading.yaml instruments     defaults)
    -                        `_ws_loop`:                                                                                                                         
    -                  `main.py`:                  `config.to_dict()`                `config.trading`
  - **DoD Verification**:
    -                                                                  
    -                                                                            : `['btcusdt', 'ethusdt']`
    -     WebSocket                           :              bookTicker    trade                 
    -                                                                           
...
- [ ] **Part C**:                            production checklist (deployment, monitoring, rollback)

###      Task 02:                      Live End-to-End Test (FSMP-PROD-PREP-T02)

- [ ] **Part A**:                             FSMP-EXECUTE-T05-LIVE    extended runtime
  - **Goal**:                  market ticks                  30+             
  - **Success**:                         MARKET_TICK     FEATURES     RISK     DECISION     BRIDGE     EXECUTION
...
- [x] **FSMP-P1-T02**: 3 FSM flows (open/manage/close)    shadow-mode     COMPLETED [AURORA_FSM_LIFECYCLE_V1 + AURORA_FSM_TEST_FIX_V1]
...
- [x] **FSMP-PERFECT-T16**: Account Connector Coverage Gaps     DONE; 27/27 tests pass (100%); coverage 78%   92% (+14%) **     TARGET EXCEEDED!** [2025-01-17]
  ...
  -        Remaining gaps: Lines 18-20 (import fallback for requests), 114-117 (partial monitor loop), 145-151 (partial _get_account_info)     12 lines, 8%
...
- [x] **FSMP-PERFECT-T17**: Market Data Connector Coverage Gaps     DONE; 20/20 tests pass (100%); coverage 79%   90% (+11%) **     TARGET REACHED!** [2025-01-17]
  ...
  -        Remaining gaps: Lines 19-21 (import fallback for unicorn), 101-108 (HAS_UNICORN guard), 123 (while loop), 190-191 (edge case)     13 lines, 10%
