...
**WHY**: PACK EXP-4: Decision QoS rate-limit                               intent spam        exposure block; PACK EXP-5: Normalized Reject Reasons                                     error codes.

**STATUS**:     **PASS** (QoS                       spam, NRR                          errors,                                     )

**IMPLEMENTATION SUMMARY**:
- **PACK EXP-4: Decision QoS & Anti-Intent Spam**
  - QoS Configuration:              qos                 config/aurora/trading.yaml (exposure_block_cooldown_sec=10, symbol_cooldown_sec=3, max_intents_per_minute_per_symbol=6)
  - Schema Validation:                  config/_schemas/aurora_trading.schema.json    qos                      (1-300s, 1-60s, 1-60 ranges)
  - QoS Logic:                        _qos_allow(), _update_symbol_cooldown(), _update_intent_count(), _handle_exposure_block()    DecisionMaking
  - Exposure Block Handling:                                           exposure limit                         10s cooldown
  - Rate Limiting: Per-symbol rate limiting    sliding window (60s)      intent counting (max 6/min)
  - Symbol Cooldowns:                    cooldowns                                                                (3s)
  - Integration: QoS                                              _make_decision_for_symbol()    NRR             
  - Tests:                  test_decision_making_qos.py                                    QoS                    (8             )

- **PACK EXP-5: Normalized Reject Reasons**
  - NRR Module:                  normalized_reject_reasons.py    14                          error              (NRR-001      NRR-014)
  - Regex Patterns:                        pattern matching        Binance API                                                           
  - Error Mapping: Insufficient balance, invalid params, market closed, exposure limits, rate limits, network errors, timeouts
  - Integration:        reject reasons    DecisionMaking                                            NRR                           
  - Unknown Fallback: NRR-999                                             UNKNOWN_ERROR           
  - Tests:                  test_normalized_reject_reasons.py    pattern matching                (7             )

**TECHNICAL DETAILS**:
- **QoS State Management**: defaultdict        symbol_cooldowns, symbol_intent_counts    window tracking
- **Exposure Block Detection**: Regex pattern matching      'exposure'    risk_params                           cooldown
- **Rate Limiting Algorithm**: Sliding window    count reset        elapsed >= 60s
- **NRR Pattern Matching**: re.search    case-insensitive flags                         matching
- **Fail-Closed**: QoS blocks decisions        rate limit/exposure,                         normal flow
- **Performance**: Time-based checks (time.time())        heavy computations
...
**FILES MODIFIED**:
- `config/aurora/trading.yaml`:              decision.qos             
- `config/_schemas/aurora_trading.schema.json`:              qos properties                   
...
- `TODO.md`:              PACK EXP-4/5 completion status

**NEXT**: PACK EXP-6                                             exposure guard reliability

## 2025-10-30 | RID: PACK-EXP-3-TELEMETRY |     PASS     Telemetry & Metrics (Prometheus + FSM hooks)

**WHY**: PACK EXP-3: Telemetry & Metrics -                                 Prometheus                                                                   , /metrics endpoint, FSM hooks                               guard performance.

**STATUS**:     **PASS** (                                         , /metrics             ,        hooks                       ,                              )

**IMPLEMENTATION SUMMARY**:
- **Metrics Module**:                  `vfoundation/apps/reference/telemetry/metrics.py`    gauges        exposure (equity/positions/pending/limit)      counters                   (guard_rejects, pending_expired, manage_skipped, orders_placed/filled, decision_rate_limited)
- **API Endpoint**:              `/metrics`    `apps/reference/api/main.py`    Response        Prometheus                (production mode)
- **FSM Hooks**:                                          `vfoundation/apps/reference/domains/execution_position/fsm.py` - update_exposure      PORTFOLIO_STATE_UPDATED, inc_exposure_guard_block      ERR:OPEN    PORTFOLIO_EXPOSURE_LIMIT
- **Manage Hook**:              kill-switch    `fsm_manage.py` - inc_manage_skipped          auto_manage_enabled=false
- **Adapter Hooks**:                           `binance_execution_adapter.py` - inc_order_placed                               place, inc_order_filled    WebSocket handler        FILLED               
- **Fail-Closed**:                                                prometheus_client, graceful degradation
- **Tests**:                  unit/integration            - test_metrics_update.py, test_metrics_endpoint.py, test_exposure_guard_reject_counter

**TECHNICAL DETAILS**:
- **Metrics Format**: Prometheus-compatible    HELP/TYPE/# VALUE               
- **Exposure Calculation**: equity_usd * fraction = limit_usd, positions_usd + pending_usd = total exposure
- **Event Hooks**: update_exposure()                     portfolio update, counters      terminal events (ERR:OPEN, ORDER_FILLED)
- **Production Mode**: /metrics                                          TRADING_ENV != 'production' (debug API fallback)
- **Decimal Safety**: _d()                                                                        float    fallback
...
**FILES MODIFIED**:
- `requirements.txt`:              prometheus_client>=0.20.0
- `vfoundation/apps/reference/telemetry/metrics.py`: NEW (metrics module    gauges/counters)
- `apps/reference/api/main.py`:              /metrics endpoint
- `vfoundation/apps/reference/domains/execution_position/fsm.py`: hooks        exposure update      guard reject
- `vfoundation/apps/reference/domains/execution_position/fsm_manage.py`: kill-switch hook        manage skipped
- `vfoundation/apps/reference/domains/execution_position/binance_execution_adapter.py`: hooks        order placed/filled
- `tests/units/test_metrics_update.py`: NEW (unit test        metrics export)
- `tests/integration/test_metrics_endpoint.py`: NEW (integration test        /metrics)
- `tests/integration/test_exposure_release_hooks.py`:              test_exposure_guard_reject_counter

**NEXT**: PACK EXP-4: Decision QoS rate-limit        PACK EXP-5: Documentation
...
- **redis_store.py**: 220 stmt, 21 miss     **90%** (+6% from baseline, gate met)
  ...
  -        **Remaining gaps** (21 lines = 10% defensive/edge code):
...
-        Gap 10% (21 lines) = **defensive/edge code** (script_load exception fallback unmockable with fakeredis global patch, edge cases in helpers)
...
## 2025-10-14 | RID: FSMP-P2-T02 | PARTIAL PASS     Coverage 83%, mypy=20       

**WHY**: `44/44 PASS; cov=83% (fakeredis limit); mypy=20 (async/sync Redis typing)`

**STATUS**:        **PARTIAL PASS** (83% functional coverage, mypy deferred to T03)

**FINAL RESULTS**:
-     **44/44 tests PASS** (100% success rate)
-        **Coverage 83%** (goal 90%, gap 7% = fakeredis limitations)
...
-        Gap 7% = **defensive/edge code** (ImportError check, script_load None, CB internal state)
...
-        Coverage 83% (90% ideal, but 83% covers all functional + error paths; gaps = edge cases)
...
-        Coverage 74% (target 90%)     baseline established, gaps in error handling/CB/retry paths
...
- mypy: **0 errors**     (         37 warnings)
- Drift: <1%, accuracy 98%+
- WHY:    80 chars discipline

**WVR-01 RATIONALE**:
- Critical paths covered 95%+
- Platform-specific (~0.5%): Unix fcntl      Windows CI
...
- Created `.coveragerc`        platform exclusions
...
**RESULTS**:
- **Tests: 141     159 passed** (+18                        )
- **Coverage ACL**: acl.py 90%, contracts.py 97%
- **Shadow-mode**: stub exchange        testing (no live orders)
...
## 2025-01-12 | RID: FSMP-P1-INIT | P1 Baseline                 
...
- Baseline branch: `feat/p1-execpos-shadow`        P1 tasks
- Ready        FSMP-P1-T01: ACL adapter + shadow wiring

**RESULTS**:
- P0 gates passed: tests 141/141, coverage 90%, schema-lint OK
- Repository state: production-ready foundation
- Next: ACL adapter        execution_position domain
...
**WHY**:                                 (RBAC, Ed25519        DEC/CMD)      XAI-                     (why   80)

**ACTIONS**:
- **RBAC**: `/debug`    `/replay` endpoints                             `require_admin()` (403                    )
- **Signature verification**: DEC/CMD ops                                         Ed25519                (401        sig)
  - `routing.py`                                            WAL-write
  -                     `signing_ed25519.sign()` / `verify()`
- **WHY-discipline**: Router-level validation        `why > 80` chars     400        WAL-write
- **Contracts**:              CMD      `global_v2_2.yaml` ops list, regenerated `message_v1.json` schema
- **CLI**:                      `vfound dict_lint`                 `vfoundation/dictionaries/`
- **Dependencies**:                        `typer` (CLI)      `pynacl` (Ed25519 crypto)
- **Breaking change**:                  4               `test_single_flight_routing.py`                       DEC ops

**RESULTS**:
- **          : 137     141 passed** (13            security             , 4                       )
- **Coverage: 89%     90%** (517/572             )     **TARGET                   !**
- **Security coverage**: RBAC 100%, signature stub 100%, WHY-discipline validated
- **                100%                   **: idempotency, protocol, retry_cb, replay, why, rbac_abac, signing_ed25519
...
- **Artefacts**: `tests/test_security_xai_tighten.py` (13             ), `schemas/message_v1.json`
