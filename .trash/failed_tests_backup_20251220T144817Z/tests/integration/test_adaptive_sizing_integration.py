...
@pytest.mark.parametrize(
    "regime, modifier, expected_size_usd",
    [
...
def test_adaptive_sizing_integration_across_regimes(
    decision_domain_for_sizing, regime, modifier, expected_size_usd
):
    """
    ...
    assert actual_size <= Decimal("10000"), (
        f"Position size should not exceed liquidity cap, got {actual_size}"
    )
