...
class DecisionMaking:
    ...
    def __init__(self, fsm: "FSMCore", config: dict[str, Any]) -> None:
        ...
        self.prev_equity: Optional[decimal.Decimal] = (
            None  # Track previous equity for logging (FIXED BUG-P2-001: removed duplicate)
        )
        ...
        try:
            ...
            self.logger.warning(
                f"Invalid liquidity_based_cap_usd in config, using default 10000: {e}"
            )
    ...
    def on_features(self, event: Message) -> None:
        ...
        self.dlog.write(
            ...
            {"symbol": event.pld.get("symbol", "unknown"), "keys": list(feats.keys())},
        )
    ...
    def on_portfolio(self, event: Message) -> None:
        ...
        self.dlog.write(
            ...
            getattr(event, "rid", None),
            {
                "equity": str(event.pld.get("equity_free_usdt", "0")),
                "positions_count": len(event.pld.get("positions", [])),
            },
        )
        self._try_make_decision()
    ...
    def _try_make_decision(self) -> None:
        ...
        try:
            ...
            if not all([self.latest_features, self.latest_risk, self.latest_portfolio]):
                ...
                self.logger.info(
                    f"[STATE_SYNC] Waiting for fresh data: missing {', '.join(missing_parts)}"
                )
                return
            ...
            if self.prev_equity is not None and self.prev_equity != equity:
                self.logger.info(
                    f"Equity updated: ${float(self.prev_equity):.2f} → ${float(equity):.2f}"
                )
            self.prev_equity = equity
            if equity <= decimal.Decimal("0"):
                self.logger.warning(
                    f"Trade intent rejected for {symbol}: Invalid equity ${equity:.2f}"
                )
                self.dlog.write("DECISION_SKIP", rid, {"symbol": symbol, "reason": "ZERO_EQUITY"})
            ...
            if not risk_params.get("is_trading_allowed", False):
                self.logger.info(
                    f"Trade intent rejected for {symbol}: Trading not allowed by risk manager."
                )
                self.dlog.write(
            ...
            signal_score = (
                (obi * obi_weight) + (tfi * tfi_weight) + (absorption * absorption_weight)
            )
            ...
            if signal_score > signal_threshold:
                ...
                self.logger.info(
                    f"Trade intent rejected for {symbol}: Neutral signal score {signal_score:.3f}"
                )
                self.clear_internal_state()
                return
            ...
            self.logger.info(
                f"[POSITION_GATE] Checking positions for {symbol}: total_positions={len(positions_list)}"
            )
            ...
            if existing_position:
                ...
                if current_qty > decimal.Decimal("1e-9") and intended_side == "sell":
                    self.logger.info(
                        f"[POSITION_GATE] ✅ Allowing SELL for {symbol}: closing existing LONG position"
                    )
                elif current_qty < -decimal.Decimal("1e-9") and intended_side == "buy":
                    self.logger.info(
                        f"[POSITION_GATE] ✅ Allowing BUY for {symbol}: closing existing SHORT position"
                    )
                else:
                    # Same direction trade - block to prevent accumulation
                    self.logger.warning(
                        f"[POSITION_GATE] ❌ Trade intent BLOCKED for {symbol}: Same direction as existing position (qty={current_qty}, intended_side={intended_side})"
                    )
                    self.clear_internal_state()
                    return
            else:
            ...
            full_kelly = (
                (p - (decimal.Decimal("1") - p) / payoff_ratio_r)
                if payoff_ratio_r > decimal.Decimal("0")
                else decimal.Decimal("0")
            )
            kelly_cap = decimal.Decimal(
                str(self.config.get("system", {}).get("kelly", {}).get("fraction_cap", "0.85"))
            )
            ...
            cvar_trade_usd = (
                equity
                * decimal.Decimal(str(risk_budgets["trade_cvar95_max_bps"]))
                / decimal.Decimal("10000")
            )
            cvar_session_usd = (
                equity
                * decimal.Decimal(str(risk_budgets["session_cvar95_max_bps"]))
                / decimal.Decimal("10000")
            )
            ...
            p_calib_metrics = decision_config.get(
            ...
            self.logger.debug(
                f"[QTY_DIAG]   - Probability (p): {p} (raw={p_raw}, cal={p_cal}, min={min_prob}, max={max_prob})"
            )
            self.logger.debug(
                f"[QTY_DIAG]   - Kelly Used: {kelly_used} (full={full_kelly}, alpha={kelly_alpha}, cap={kelly_cap})"
            )
            ...
            kelly_conservative_factor = decimal.Decimal(
                str(sizing_config["kelly_conservative_factor"])
            )
            ...
            self.logger.debug(
                f"[QTY_DIAG]   - Kelly Conservative Factor: {kelly_conservative_factor}"
            )
            ...
            liquidity_based_cap = decimal.Decimal(
                str(sizing_config.get("liquidity_based_cap_usd", "10000"))
            )
            notional_cap = min(
                kelly_based_cap, cvar_based_cap, liquidity_based_cap, default_notional_cap
            )
            ...
            self.logger.debug(
                f"[QTY_DIAG]   - Final Notional Cap: ${notional_cap} (minimum of all caps)"
            )
            ...
            self.logger.debug(
                f"[QTY_DIAG]   - Position Size (before regime adjustment): ${position_size / regime_size_multiplier}"
            )
            self.logger.debug(f"[QTY_DIAG]   - Regime Size Multiplier: {regime_size_multiplier}")
            self.logger.debug(
                f"[QTY_DIAG]   - Position Size (after regime adjustment): ${position_size}"
            )

            if position_size < min_position_size:
                self.logger.info(
                    f"[QTY_DIAG] ❌ Trade intent REJECTED for {symbol}: Calculated size ${position_size:.2f} is below minimum ${min_position_size:.2f}"
                )
                self.clear_internal_state()
                return

            self.logger.debug(
                f"[QTY_DIAG] ✅ Position size ${position_size} passed minimum threshold"
            )
            ...
            margin_safety_factor = decimal.Decimal(
                str(sizing_config.get("margin_safety_factor", 0.9))
            )
            ...
            self.logger.debug(
                f"[QTY_DIAG]   - Required Margin: ${required_margin} (position / leverage)"
            )
            ...
            if required_margin > max_usable_margin:
                ...
                self.logger.warning(
                    f"[QTY_DIAG]   - Desired: ${position_size:.2f}, Required margin: ${required_margin:.2f}"
                )
                self.logger.warning(
                    f"[QTY_DIAG]   - Available: ${available_margin:.2f}, Max usable: ${max_usable_margin:.2f}"
                )
                self.logger.warning(
                    f"[QTY_DIAG]   - Capped to: ${capped_position_size:.2f} (usable_margin * leverage)"
                )
                ...
                self.logger.debug(
                    f"[QTY_DIAG] ✅ Sufficient margin available (required ${required_margin:.2f} <= usable ${max_usable_margin:.2f})"
                )
            ...
            if self.latest_features and "price" in self.latest_features.get("features", {}):
                ...
                self.logger.debug(
                    f"[QTY_DIAG] Price source: latest_features['price'] = {price_ref}"
                )
                ...
                self.logger.debug(
                    f"[QTY_DIAG] Price source: latest_portfolio['last_price'] = {price_ref}"
                )
            else:
                ...
                if bid > 0 and ask > 0:
                    price_ref = (bid + ask) / 2
                    self.logger.debug(
                        f"[QTY_DIAG] Price source: mid(bid={bid}, ask={ask}) = {price_ref}"
                    )
                else:
            ...
            if price_ref <= 0:
                self.logger.error(
                    f"[QTY_DIAG] ❌ CRITICAL: No valid price reference available for {symbol}. Cannot make trading decision without reliable price data."
                )
                self.clear_internal_state()
            ...
            if qty < min_qty:
                self.logger.info(
                    f"[QTY_DIAG] ❌ Trade intent REJECTED for {symbol}: Calculated qty {qty:.6f} below minimum {min_qty}"
                )
                ...
                self.logger.info(
                    f"[QTY_DIAG]   - Step size too large ({step_size}) caused excessive floor rounding"
                )
                self.clear_internal_state()
                return
            ...
            if self.latest_regime and self.latest_regime.get("symbol") == symbol:
                ...
                if (
                    current_regime in ["HIGH_VOLATILITY", "LOW_VOLATILITY"]
                    and current_regime in sizing_modifiers
                ):
                    ...
                    if qty < min_qty:
                        self.logger.info(
                            f"Trade intent rejected for {symbol}: Modified qty {qty:.6f} below minimum {min_qty}"
                        )
                        self.clear_internal_state()
                        return
                ...
                # PRIORITY 2: Mean reversion sizing (backward compatibility with existing logic)
                    ...
                    if qty < min_qty:
                        self.logger.info(
                            f"Trade intent rejected for {symbol}: Reduced qty {qty:.6f} below minimum {min_qty}"
                        )
                        self.clear_internal_state()
                        return
            ...
            min_liq_distance_pct = decimal.Decimal(
                str(risk_config.get("min_liquidation_distance_pct", 5.0))
            )
            mmr = decimal.Decimal(
                str(risk_config.get("maintenance_margin_rate", 0.004))
            )  # 0.4% default
            ...
            imr = (
                decimal.Decimal("1") / leverage if leverage > 0 else decimal.Decimal("1")
            ...
            self.logger.debug(
                f"[LIQUIDATION_GUARD]   - Initial Margin Ratio: {imr:.4f} (1/leverage)"
            )
            self.logger.debug(f"[LIQUIDATION_GUARD]   - Maintenance Margin Rate: {mmr:.4f}")
            self.logger.debug(
                f"[LIQUIDATION_GUARD]   - Calculated Liquidation Price: ${liq_price:.2f}"
            )
            self.logger.debug(
                f"[LIQUIDATION_GUARD]   - Distance to Liquidation: ${distance_to_liq:.2f} ({distance_pct:.2f}%)"
            )
            self.logger.debug(
                f"[LIQUIDATION_GUARD]   - Minimum Required Distance: {min_liq_distance_pct:.2f}%"
            )
            ...
            if idempotency_config.get("enabled", False):
                ...
                key_template = idempotency_config.get(
                    "key_template", "{symbol}:{side}:{ts_bucket_ms}"
                )
                idem_key_raw = key_template.format(
                    symbol=symbol, side=side.lower(), ts_bucket_ms=ts_bucket
                )
                ...
                self.logger.info(
                    f"[IDEMPOTENCY] Generated key: {idempotent_key} (from: {idem_key_raw})"
                )
            else:
            ...
            trade_intent_payload = {
                ...
                "risk_budget": {
                    "trade_cvar95_max_bps": str(
                        risk_budgets["trade_cvar95_max_bps"]
                    ),  # Preserve Decimal precision as string
                    "session_cvar95_max_bps": str(
                        risk_budgets["session_cvar95_max_bps"]
                    ),  # Preserve Decimal precision as string
                },
                "size": {
            ...
            # 6. === EMIT EVENT ===
            self.logger.info(
                f"Trade intent approved: {symbol} {side} p={float(p):.3f} size=${float(position_size):.2f}"
            )

            self.dlog.write(
                "INTENT_PROPOSED",
                rid,
                {"symbol": symbol, "side": side, "qty": str(qty), "price_ref": str(price_ref)},
            )

            # PRECISION VERIFICATION: Log payload sample to verify str() preservation
            self.logger.info(
                f"PRECISION CHECK - order.qty: {trade_intent_payload['order']['qty']} (type: {type(trade_intent_payload['order']['qty']).__name__})"
            )
            self.logger.info(
                f"PRECISION CHECK - order.price: {trade_intent_payload['order']['price']} (type: {type(trade_intent_payload['order']['price']).__name__})"
            )
            self.logger.info(
                f"PRECISION CHECK - order.price_ref: {trade_intent_payload['order']['price_ref']} (type: {type(trade_intent_payload['order']['price_ref']).__name__})"
            )
            ...
            self.logger.critical(
                f"Configuration key missing: {e}. System cannot make decisions. Please check trading.yaml. Halting decision."
            )
    ...
    def clear_internal_state(self) -> None:
        ...
        self.latest_portfolio = (
            None  # FIXED: Clear portfolio to force fresh data (AURORA_STATE_SYNC_V1)
        )
    ...
    def _apply_caps(
        self, proposed_size: decimal.Decimal, cvar_cap: decimal.Decimal
    ) -> decimal.Decimal:
        """
