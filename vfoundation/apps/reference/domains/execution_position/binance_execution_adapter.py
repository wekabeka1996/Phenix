...
class BinanceExecutionAdapter(AbstractExecutionAdapter):
    ...
    def __init__(self, shadow_mode: bool = False, fsm_core: Optional[Any] = None):
        ...
        self.use_testnet = os.environ.get("USE_TESTNET", "1") == "1" or os.environ.get("USE_TESTNET", "").lower() == "true"
        ...
        if not shadow_mode and (not self.api_key or not self.api_secret):
            logger.warning("[BinanceAdapter] API credentials not found in environment, falling back to shadow mode")
            self.shadow_mode = True
        
        logger.info(f"[BinanceAdapter] Initialized with shadow_mode={self.shadow_mode}, testnet={self.use_testnet}, ws_enabled={fsm_core is not None}")
    ...
    def _websocket_loop(self) -> None:
        ...
        while self.ws_running:
            try:
                ...
                if self.ws_running:
                    logger.info(f"[BinanceAdapter] Retrying WebSocket connection in {self.ws_reconnect_delay}s")
    ...
    def _establish_websocket_connection(self) -> None:
        ...
        try:
            ...
            ws_url = f"wss://fstream.binance.com/ws/{self.ws_listen_key}" if not self.use_testnet else f"wss://stream.binancefuture.com/ws/{self.ws_listen_key}"
    ...
    def _refresh_listen_key(self) -> None:
        ...
        if resp.ok:
            ...
            logger.warning(f"[BinanceAdapter] Failed to refresh listen key: HTTP {resp.status_code}")
    ...
    def _handle_order_trade_update(self, msg: Dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.info(f"[BinanceAdapter] ORDER_TRADE_UPDATE: {symbol} {client_order_id} status={order_status}")
            
            # Map order status to FSM event
            if order_status in ["FILLED", "PARTIALLY_FILLED"]:
                event_name = "EVT:TRADE_EXECUTED" if order_status == "FILLED" else "EVT:ORDER_UPDATED"
            elif order_status in ["CANCELED", "EXPIRED", "REJECTED"]:
                event_name = "EVT:ORDER_REJECTED"
            else:
                event_name = "EVT:ORDER_UPDATED"
            ...
            payload = {
                ...
                "raw_ws_data": order_data
            }
            
            # Emit FSM event
            if self.fsm_core:
                self.fsm_core.emit(event_name, payload, f"WS_ORDER_UPDATE_{order_status}")
                logger.info(f"[BinanceAdapter] Emitted {event_name} for {symbol} {client_order_id}")
            
        except Exception as e:
            logger.error(f"[BinanceAdapter] Error processing ORDER_TRADE_UPDATE: {e}", exc_info=True)
    ...
    def _handle_account_update(self, msg: Dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.info(f"[BinanceAdapter] ACCOUNT_UPDATE: {len(balances)} balances, {len(positions)} positions")
        ...
        try:
            ...
            if resp.ok:
                ...
                if drift_ms > 500:  # More than 500ms drift
                    ...
                    logger.debug(f"[BinanceAdapter] Time sync completed, offset: {self.server_time_offset:.1f}ms")
            else:
                logger.warning(f"[BinanceAdapter] Failed to sync time with server: HTTP {resp.status_code}")
    ...
    def _reconcile_state(self) -> None:
        ...
        try:
            ...
            logger.info(f"[BinanceAdapter] Reconciliation complete: {len(open_orders)} orders, {len(positions)} positions")
    ...
    def _get_open_orders(self) -> list:
        ...
        if resp.ok:
            ...
            logger.error(f"[BinanceAdapter] Failed to get open orders: HTTP {resp.status_code} {resp.text}")
            return []
    ...
    def _get_positions(self) -> list:
        ...
        if resp.ok:
            ...
            logger.error(f"[BinanceAdapter] Failed to get positions: HTTP {resp.status_code} {resp.text}")
            return []
    ...
    def _get_account_balance(self) -> list:
        ...
        if resp.ok:
            ...
            logger.error(f"[BinanceAdapter] Failed to get balance: HTTP {resp.status_code} {resp.text}")
            return []

    def _emit_reconciliation_events(self, open_orders: list, positions: list, balances: list) -> None:
        """
        ...
        logger.info(f"[BinanceAdapter] Reconciliation state: {len(open_orders)} open orders, {len(positions)} positions, {len(balances)} balances")
    ...
    def initialize_margin_settings(self, instruments_config: Dict[str, Dict]) -> None:
        ...
        """
        if self.shadow_mode:
            logger.info("[BinanceAdapter] Shadow mode enabled - skipping margin settings initialization")
            return
        ...
        for symbol, config in instruments_config.items():
            ...
            if margin_type is None:
                logger.warning(f"[BinanceAdapter] Margin type not configured for {symbol}, skipping")
                continue
            ...
            try:
                ...
                logger.info(f"[BinanceAdapter] Successfully configured {symbol}: leverage={leverage}x, margin_type={margin_type}")
            
            except Exception as e:
                # Log error but continue with other instruments
                logger.error(f"[BinanceAdapter] Failed to set margin settings for {symbol}: {e}", exc_info=True)
    ...
    def place_order(self, dec_msg: Message) -> Dict[str, object]:
        ...
        if dec_msg.op != "DEC" or dec_msg.verb != "OPEN":
            raise ValueError(f"Invalid message type: expected DEC:OPEN, got {dec_msg.op}:{dec_msg.verb}")
        ...
        logger.info(f"[BinanceAdapter] Processing DEC:PLACE_ORDER for {binance_symbol} {binance_side} {binance_qty} {order_type}")
        ...
        if not reduce_only:
            ...
            guard_result = {"passed": True, "order_id": "bracket-order", "why_codes": ["reduce_only_bracket"]}

        # Execute order
        try:
            if self.shadow_mode:
                # Shadow mode: simulate success
                logger.info(f"[BinanceAdapter] SHADOW MODE: simulated {order_type} order for {binance_symbol}")
                return self._create_success_feedback(symbol, f"shadow-{order_type.lower()}-123", guard_result["why_codes"])
            else:
                # Live execution
                result = self._place_binance_order(
                    binance_symbol, binance_side, binance_qty, order_type, 
                    price, stop_price, reduce_only, idempotent_key
                )
                
                return self._create_success_feedback(symbol, result.get("orderId", "unknown"), guard_result["why_codes"])
    ...
    def cancel_order(self, dec_msg: Message) -> Dict[str, object]:
        ...
        try:
            ...
            if self.shadow_mode:
                ...
                return self._create_success_feedback("cancelled", order_id, "shadow_mode")
            
            # Cancel order via API
            result = self._cancel_binance_order(order_id)
            
            if result.get("status") == "CANCELED":
                return self._create_success_feedback("cancelled", order_id, "api_cancel")
            else:
                return self._create_error_feedback("unknown", f"Cancel failed: {result.get('msg', 'unknown error')}")
    ...
    def _place_binance_order(
        ...
        if not resp.ok:
            ...
            if error_code == -1021:
                ...
                logger.warning(f"[BinanceAdapter] Timestamp error (-1021), syncing time and retrying: {error_msg}")
                self._sync_time_with_server()
                ...
                if not resp.ok:
                    logger.error(f"[BinanceAdapter] Order still failed after retry: HTTP {resp.status_code} {data}")
                ...
                # Rate limit exceeded
                logger.warning(f"[BinanceAdapter] Rate limit exceeded (-429), implementing backoff: {error_msg}")
        ...
        logger.info(f"[BinanceAdapter] Order placed successfully: orderId={order_id} status={status}")
        
        return data

    def _create_success_feedback(self, instrument: str, order_id: str, why_codes: list[str]) -> Dict[str, object]:
    ...
    def _create_rejected_feedback(self, instrument: str, order_id: str, why_codes: list[str]) -> Dict[str, object]:
