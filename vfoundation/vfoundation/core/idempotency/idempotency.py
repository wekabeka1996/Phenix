...
class IdempotencyStore:
    def __init__(
        ...
        self._cache = MonotonicTTLCache[str, Any](max_entries=max_entries, default_ttl_ms=default_ttl_ms)
        self.default_ttl_ms = default_ttl_ms
        self.idem_pending_ttl_ms = idem_pending_ttl_ms or (default_ttl_ms * 2)  # Default: 2x result TTL
        ...
        self._inflight: Dict[str, tuple[InflightState, Optional[Any], int]] = {}  # key -> (state, result, expire_monotonic_ns)
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        metrics.update({
            ...
            "idem_pending_ttl_near_expiry_total": self._metrics.get("idem_pending_ttl_near_expiry_total", 0),
        })
        return metrics
    ...
    def begin(self, idempotent_key: str) -> Dict[str, bool]:
        ...
        with self._lock:
            ...
            pending_count = sum(1 for state, _, _ in self._inflight.values() if state == InflightState.PENDING)
            ...
            self._cache.set(idempotent_key, "__PENDING__", ttl_ms=self.idem_pending_ttl_ms, pinned=True)
    ...
    def complete(self, idempotent_key: str, result: Any, ttl_ms: Optional[int] = None) -> None:
        ...
        with self._lock:
            ...
            if idempotent_key in self._inflight:
                self._inflight[idempotent_key] = (InflightState.DONE, result, time.monotonic_ns() + done_ttl_ms * 1_000_000)
