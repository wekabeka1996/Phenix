...
def schema_gen(from_pydantic: bool = True) -> None:
    ...
    (out / "message_v1.json").write_text(json.dumps(msg_schema, ensure_ascii=False, indent=2), encoding="utf-8")
    typer.echo("Schemas generated: schemas/message_v1.json")

@app.command("dict")
def dict_lint(global_: bool = typer.Option(False, "--global"), domain: bool = typer.Option(False, "--domain")) -> None:
    ok = True
    if global_:
...
def simulate_flow(file: pathlib.Path) -> None:
    rid = str(uuid.uuid4())
    msg = Message(op="ASK", verb="EVAL", src="risk_strategy", dst="execution_position", rid=rid, why="simulate")
    wal.append(msg.model_dump())
    dec = Message(op="DEC", verb="EVAL", src="risk_strategy", dst="execution_position", rid=rid, why="ok")
...
def drift_batch(
    ...
    window_sec: float = typer.Option(1.0, "--window-sec", help="Time window for matching (seconds)"),
    output: pathlib.Path = typer.Option(None, "--output", "-o", help="Output JSON path")
) -> None:
    """
    ...
    spec: ModuleSpec | None = importlib.util.spec_from_file_location("drift_monitor", drift_monitor_path)
