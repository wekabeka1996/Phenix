...
_TARGET = "benchmarks/analyze_alpha_performance.py"
...
if __name__ == "__main__":
    ...
    def analyze(self, horizon_sec: int = 300) -> Dict:
        ...
        results = defaultdict(
            lambda: {"correct": 0, "total": 0, "return_sum": 0.0})
        ...
        for event in self.alpha_events:
            ...
            if is_bullish and ret > 0:
                is_correct = True
            elif is_bearish and ret < 0:
                is_correct = True
            ...
            if abs(score) > 0.05:
                key = (model, symbol)
                results[key]["total"] += 1
                results[key]["return_sum"] += ret * \
                    (1 if is_bullish else -1)  # Long/Short return
    ...
    def generate_markdown(self, results, conf_results, out_path):
        ...
        with open(out_path, 'w') as f:
            ...
            ), key=lambda x: x[1]['correct']/x[1]['total'] if x[1]['total'] > 0 else 0, reverse=True)
            ...
            for (model, symbol), stats in sorted_res:
                ...
                f.write(
                    f"| {model} | {symbol} | {total} | **{acc:.1f}%** | {avg_ret:.2f} bp |\n")

            f.write("\n## 2. Confidence Analysis\n\n")
            f.write("| Model | Confidence | Events | Hit Rate |\n")
            f.write("|---|---|---|---|\n")

            sorted_conf = sorted(conf_results.items(),
            ...
            if sorted_res:
                ...
                worst = sorted_res[-1]
                f.write(
                    f"- **Best:** {best[0][0]} on {best[0][1]} ({best[1]['correct']/best[1]['total']*100:.1f}%)\n")
                f.write(
                    f"- **Worst:** {worst[0][0]} on {worst[0][1]} ({worst[1]['correct']/worst[1]['total']*100:.1f}%)\n")
