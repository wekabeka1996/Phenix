...
if __name__ == "__main__":
    ...
    for date_str in dates:
        ...
        print(f"   Loaded {sample_count} samples with {data.shape[1]} features")
        ...
        # Train (quiet mode)
        w_rewards = train_on_real_data("Western", western_bot, data, returns, episodes=20, verbose=False)
        s_rewards = train_on_real_data("Soviet", soviet_bot, data, returns, episodes=20, verbose=False)
        ...
        print(f"   Advantage:   {all_soviet_rewards[date_str] - all_western_rewards[date_str]:+.4f} bps\n")
    ...
    print(f"  Min/Max:       {np.min(western_values):.4f} / {np.max(western_values):.4f}")
    
    print(f"\nSoviet-Western Bot (Spectral):")
    print(f"  Mean Profit:   {np.mean(soviet_values):.4f} bps")
    print(f"  Std Dev:       {np.std(soviet_values):.4f}")
    print(f"  Min/Max:       {np.min(soviet_values):.4f} / {np.max(soviet_values):.4f}")
    
    advantage_mean = np.mean(soviet_values) - np.mean(western_values)
    advantage_pct = (advantage_mean / abs(np.mean(western_values))) * 100 if np.mean(western_values) != 0 else 0
    
    print(f"\n🏆 SOVIET ADVANTAGE:")
    print(f"  Absolute:      {advantage_mean:+.4f} bps")
    print(f"  Relative:      {advantage_pct:+.2f}%")
    
    win_rate = sum(1 for w, s in zip(western_values, soviet_values) if s > w) / len(dates_sorted) * 100
    ...
    width = 0.35
    ax1.bar(x - width/2, western_values, width, label='Western Bot (Baseline)', color='gray', alpha=0.7)
    ax1.bar(x + width/2, soviet_values, width, label='Soviet-Western Bot (Spectral)', color='red', alpha=0.8)
    ax1.set_xlabel('Date')
    ax1.set_ylabel('Avg Profit (bps)')
    ax1.set_title('Daily Profit Comparison: DOGEUSDT 15m (2026-01-12 to 2026-01-18)')
    ax1.set_xticks(x)
    ...
    plt.tight_layout()
    plt.savefig('multi_day_benchmark_results.png', dpi=150, bbox_inches='tight')
    print(f"\n✓ Plot saved as multi_day_benchmark_results.png")
    plt.show()
    
    # Save detailed report
    with open('multi_day_benchmark_report.txt', 'w', encoding='utf-8') as f:
        f.write("="*70 + "\n")
        ...
        for date, w_val, s_val in zip(dates_sorted, western_values, soviet_values):
            adv = s_val - w_val
            f.write(f"{date}: Western={w_val:>7.4f} bps | Soviet={s_val:>7.4f} bps | Adv={adv:>+7.4f} bps\n")
