...
def test_simulated_adapter():
    ...
    assert "sim_" in result["exchange_order_id"], "❌ Order ID should have 'sim_' prefix"
    ...
    precision_msg = Message(
        ...
        src="test",
        dst="execution",
        pld={"symbol": "ETHUSDT", "qty": high_precision_qty, "side": "sell", "price": "4000.0"}
    )
    result = adapter.place_order(precision_msg)
    
    assert result["filled_qty"] == high_precision_qty, "❌ Decimal precision must be preserved"
    assert Decimal(result["filled_qty"]) == Decimal(high_precision_qty), "❌ Decimal conversion failed"
    ...
    print(f"   ✅ Generated unique IDs: {result1['exchange_order_id']} != {result2['exchange_order_id']}")
    print()
