...
def generate_plan_report(changes: List[Dict[str, Any]], output_file: Path) -> None:
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        ...
        for change in changes:
            value_str = str(change.get(
                'value', change.get('existing_value', 'N/A')))
            f.write(
                f"| {change['key']} | {change['action']} | {value_str} | {change['file']} | {change['path']} |\n")

        added = len([c for c in changes if c['action']
                    in ('add_default', 'add_null')])
...
def main():
    parser = argparse.ArgumentParser(
        description='Auto-fill config defaults into YAML')
    parser.add_argument(
        '--inventory', default='reports/TASK20_defaults_inventory.json', help='Inventory JSON file')
    parser.add_argument(
        '--mapping', default='tools/config_default_path_map.yaml', help='Mapping YAML file')
    parser.add_argument('--config-dir', default='config/aurora',
                        help='Target config directory (default: config/aurora)')
    parser.add_argument('--include-inventory-defaults', action='store_true', default=False,
                        help='Also apply TASK21A-style default autofill from inventory (unsafe for TASK23 unless inventory is clean)')
    parser.add_argument('--dry-run', action='store_true', help='Dry run mode')
    parser.add_argument('--apply', action='store_true', help='Apply changes')
    parser.add_argument('--fail-on-unknown-mapping',
                        action='store_true', help='Fail if mapping missing')
    parser.add_argument('--autofill-optional-nulls', action='store_true', default=False,
                        help='Autofill explicit nulls for Optional-required fields (TASK23.FIX.A)')
    parser.add_argument(
        '--plan-report', default='reports/TASK23FIX_optional_null_plan.md', help='Plan report output')
    parser.add_argument(
        '--applied-report', default='reports/TASK23FIX_optional_null_applied.md', help='Applied report output')
    ...
    inventory = load_inventory(
        inventory_file) if args.include_inventory_defaults else []
    mapping = load_mapping(
        mapping_file) if args.include_inventory_defaults else {}
    ...
    if args.autofill_optional_nulls:
        changes.extend(apply_optional_null_autofill(
            config_dir=config_dir, dry_run=not args.apply))
