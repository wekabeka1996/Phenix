...
def resolve_normalizer_context(config, override: Optional[Path]) -> NormalizerContext:
    configured_scope = str(getattr(config.ingest, "normalization_scope", "global")).lower()
...
def encode_symbol_samples(
    ...
    with torch.no_grad():
        for symbol, records in symbol_samples.items():
            if not records:
                continue
            arr = np.stack([r.norm_vec for r in records]).astype(np.float32, copy=False)
            all_mu = []
            for i in range(0, arr.shape[0], batch_size):
                batch = torch.from_numpy(arr[i : i + batch_size]).to(device=device)
            ...
            total += len(records)
            print(f"[INFO] Encoded {symbol}: {len(records):,} vectors", flush=True)
    return total
...
def settle_with_horizon(
    ...
    for symbol, records in symbol_samples.items():
        ...
        for idx in range(len(records) - horizon):
            ...
            regime_id = int(
                labeler.compute_realized_regime(cur.raw_features, fut.raw_features)
            )
...
def compute_centroid_stats(
    ...
    dist_matrix = np.full((len(REGIME_IDS), len(REGIME_IDS)), np.nan, dtype=np.float64)
...
def distance_matrix_markdown(dist_matrix: np.ndarray) -> str:
    header = "| Regime | " + " | ".join(REGIME_LABELS[r] for r in REGIME_IDS) + " |"
...
def write_report(
    ...
    lines.append("")
    lines.append(f"- Requested tail lines across `logs/features/*.log`: **{lines_requested:,}**")
    lines.append(f"- Labeling horizon (`horizon_bars`): **{horizon}**")
    lines.append(f"- Normalization scope used for encoding: **{normalization_scope}**")
    ...
    if viz_missing:
        ...
        lines.append("- Install command: `pip install scikit-learn matplotlib`")
        lines.append("")
...
def main() -> int:
    ...
    horizon = int(args.horizon_bars if args.horizon_bars is not None else config.oracle.horizon_bars)
    if horizon < 1:
    ...
    if not feature_files:
        print(f"[ERROR] No feature logs found in {args.features_dir}", file=sys.stderr)
        return 2
    budgets = allocate_tail_budgets(feature_files, int(args.max_lines), horizon)
    print(
        "[INFO] Feature files: "
    ...
    df = settle_with_horizon(symbol_samples=symbol_samples, labeler=labeler, horizon=horizon)
    ...
    print(f"[INFO] Saved latent dataset: {args.output_dataset} ({len(df):,} rows)", flush=True)

    counts = regime_counts(df)
    centroids, cluster_stats, dist_matrix = compute_centroid_stats(df, latent_cols=latent_cols)
    mr_hv_distance = float(dist_matrix[2, 3]) if np.isfinite(dist_matrix[2, 3]) else float("nan")
    mr_rms_radius = float(cluster_stats.get(2, {}).get("rms_radius", float("nan")))
    ...
    if plt is not None and pca_cls is not None:
        ...
        idx = sample_indices(coords.shape[0], int(args.max_plot_points), seed=42)
        ...
        if not args.disable_tsne and tsne_cls is not None:
            idx_tsne = sample_indices(z_all.shape[0], int(args.max_tsne_points), seed=123)
            ...
            if z_tsne.shape[0] >= 50:
                ...
                tsne_written = True
                print(f"[INFO] t-SNE plot saved: {args.output_tsne}", flush=True)
            else:
                print("[WARN] t-SNE skipped: not enough samples after subsampling.", flush=True)
    else:
        print(
    ...
    write_report(
        ...
        price_feature_mode=str(getattr(config.ingest, "price_feature_mode", "raw")),
        delta_price_mode=str(getattr(config.ingest, "delta_price_mode", "raw")),
