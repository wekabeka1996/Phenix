...
def compute_regime_metrics(result: Dict, t_peak_ms: Optional[int]) -> Dict[str, Any]:
    ...
    # Top regime
    top_regime = max(
        regime_counts, key=regime_counts.get) if regime_counts else None
...
def generate_backtest_diagnostics(run_dir: str) -> Dict[str, Any]:
    ...
    cancel_metrics, missing_artifacts = compute_cancel_stale_regime(
        order_log_path, t_peak_ms)
    ...
    # Alpha Search metrics (if plugin was active during backtest)
    alpha_data = result.get('alpha_search')
    if alpha_data and isinstance(alpha_data, dict):
        diagnostics['alpha_search'] = {
            'enabled': alpha_data.get('enabled', False),
            'mode': alpha_data.get('mode', 'unknown'),
            'providers': alpha_data.get('providers', []),
            'provider_stats': alpha_data.get('provider_stats', {}),
            'cache': alpha_data.get('cache', {}),
        }
...
def main():
    parser = argparse.ArgumentParser(
        description='Generate backtest diagnostics')
